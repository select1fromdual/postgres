  ...
  if (victim >= NBuffers) {
    ...
    if (victim == 0) {
      ...
      while (!success) {
        ...
        if (success) StrategyControl->completePasses++;
...
void StrategyFreeBuffer(BufferDesc *buf) {
  ...
  if (buf->freeNext == FREENEXT_NOT_IN_LIST) {
    ...
    if (buf->freeNext < 0) StrategyControl->lastFreeBuffer = buf->buf_id;
...
void FreeAccessStrategy(BufferAccessStrategy strategy) {
  ...
  if (strategy != NULL) pfree(strategy);
}
...
bool StrategyRejectBuffer(BufferAccessStrategy strategy, BufferDesc *buf) {
  ...
  if (strategy->btype != BAS_BULKREAD) return false;
