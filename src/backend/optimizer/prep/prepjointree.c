...
static Node *pull_up_subqueries_recurse(PlannerInfo *root, Node *jtnode, JoinExpr *lowest_outer_join,
...
static void pull_up_union_leaf_queries(Node *setOp, PlannerInfo *root, int parentRTindex, Query *setOpQuery,
                                       int childRToffset);
...
static Node *pull_up_constant_function(PlannerInfo *root, Node *jtnode, RangeTblEntry *rte,
                                       JoinExpr *lowest_nulling_outer_join, AppendRelInfo *containing_appendrel);
static bool is_simple_union_all(Query *subquery);
static bool is_simple_union_all_recurse(Node *setOp, Query *setOpQuery, List *colTypes);
static bool is_safe_append_member(Query *subquery);
static bool jointree_contains_lateral_outer_refs(PlannerInfo *root, Node *jtnode, bool restricted,
                                                 Relids safe_upper_varnos);
static void perform_pullup_replace_vars(PlannerInfo *root, pullup_replace_vars_context *rvcontext,
                                        JoinExpr *lowest_nulling_outer_join, AppendRelInfo *containing_appendrel);
...
void transform_MERGE_to_join(Query *parse) {
  ...
  if (parse->commandType != CMD_MERGE) return;
  ...
  if (jtnode == NULL) {
    ...
    *relids = frelids;
    ...
    switch (j->jointype) {
      case JOIN_INNER:
        j->quals =
            pull_up_sublinks_qual_recurse(root, j->quals, &jtlink, bms_union(leftrelids, rightrelids), NULL, NULL);
        break;
      case JOIN_LEFT:
...
static Node *pull_up_sublinks_qual_recurse(PlannerInfo *root, Node *node, Node **jtlink1, Relids available_rels1,
  ...
  if (is_andclause(node)) {
    ...
    foreach (l, ((BoolExpr *)node)->args) {
      ...
      if (newclause) newclauses = lappend(newclauses, newclause);
    }
...
static Node *pull_up_subqueries_recurse(PlannerInfo *root, Node *jtnode, JoinExpr *lowest_outer_join,
  ...
  if (IsA(jtnode, RangeTblRef)) {
     ...
     */
    if (rte->rtekind == RTE_SUBQUERY && is_simple_subquery(root, rte->subquery, rte, lowest_outer_join) &&
        (containing_appendrel == NULL || is_safe_append_member(rte->subquery)))
      return pull_up_simple_subquery(root, jtnode, rte, lowest_outer_join, lowest_nulling_outer_join,
     ...
     */
    if (rte->rtekind == RTE_FUNCTION)
      return pull_up_constant_function(root, jtnode, rte, lowest_nulling_outer_join, containing_appendrel);
...
static Node *pull_up_simple_subquery(PlannerInfo *root, Node *jtnode, RangeTblEntry *rte, JoinExpr *lowest_outer_join,
   ...
   */
  if (subquery->hasSubLinks) pull_up_sublinks(subroot);
   ...
   */
  if (lowest_nulling_outer_join != NULL) rvcontext.need_phvs = true;
   ...
   */
  perform_pullup_replace_vars(root, &rvcontext, lowest_nulling_outer_join, containing_appendrel);
  ...
  if (rte->lateral) {
    foreach (lc, subquery->rtable) {
      ...
      switch (child_rte->rtekind) {
        case RTE_RELATION:
          if (child_rte->tablesample) child_rte->lateral = true;
          break;
        case RTE_SUBQUERY:
        case RTE_FUNCTION:
        case RTE_VALUES:
...
static void make_setop_translation_list(Query *query, int newvarno, AppendRelInfo *appinfo) {
  ...
  appinfo->parent_colnos = pcolnos = (AttrNumber *)palloc0(appinfo->num_child_cols * sizeof(AttrNumber));
...
static bool is_simple_subquery(PlannerInfo *root, Query *subquery, RangeTblEntry *rte, JoinExpr *lowest_outer_join) {
   ...
   */
  if (!IsA(subquery, Query) || subquery->commandType != CMD_SELECT) elog(ERROR, "subquery is bogus");
   ...
   * anyway.
   */
  if (subquery->hasAggs || subquery->hasWindowFuncs || subquery->hasTargetSRFs || subquery->groupClause ||
      subquery->groupingSets || subquery->havingQual || subquery->sortClause || subquery->distinctClause ||
      subquery->limitOffset || subquery->limitCount || subquery->hasForUpdate || subquery->cteList)
    return false;
   ...
   */
  if (rte->security_barrier) return false;
  ...
  if (rte->lateral) {
    ...
    if (lowest_outer_join != NULL) {
      ...
      if (!bms_is_subset(lvarnos, safe_upper_varnos)) return false;
    }
  }
   ...
   */
  if (contain_volatile_functions((Node *)subquery->targetList)) return false;
...
static bool is_simple_values(PlannerInfo *root, RangeTblEntry *rte) {
   ...
   */
  if (list_length(rte->values_lists) != 1) return false;
   ...
   * pull_up_simple_values().
   */
  if (list_length(root->parse->rtable) != 1 || rte != (RangeTblEntry *)linitial(root->parse->rtable)) return false;
...
static Node *pull_up_constant_function(PlannerInfo *root, Node *jtnode, RangeTblEntry *rte,
  ...
  if (rte->funcordinality) return jtnode;

  /* Fail if RTE isn't a single, simple Const expr */
  if (list_length(rte->functions) != 1) return jtnode;
   ...
   */
  if (rtf->funccolcount != 1) return jtnode; /* definitely composite */

  functypclass = get_expr_result_type(rtf->funcexpr, &funcrettype, &tupdesc);
  if (functypclass != TYPEFUNC_SCALAR) return jtnode; /* must be a one-column composite type */

  /* Create context for applying pullup_replace_vars */
  rvcontext.root = root;
  rvcontext.targetlist = list_make1(makeTargetEntry((Expr *)rtf->funcexpr, 1, /* resno */
                                                    NULL,                     /* resname */
                                                    false));                  /* resjunk */
  rvcontext.target_rte = rte;
  ...
  rvcontext.wrap_non_vars = false;
  /* initialize cache array with indexes 0 .. length(tlist) */
  rvcontext.rv_cache = palloc0((list_length(rvcontext.targetlist) + 1) * sizeof(Node *));
...
static bool is_simple_union_all_recurse(Node *setOp, Query *setOpQuery, List *colTypes) {
  if (IsA(setOp, RangeTblRef)) {
    ...
    /* Must be UNION ALL */
    if (op->op != SETOP_UNION || !op->all) return false;
...
static bool is_safe_append_member(Query *subquery) {
  ...
  if (jtnode->fromlist == NIL && jtnode->quals == NULL) return true;
  /* Check the more general case */
  while (IsA(jtnode, FromExpr)) {
    if (jtnode->quals != NULL) return false;
    if (list_length(jtnode->fromlist) != 1) return false;
    jtnode = linitial(jtnode->fromlist);
  }
  if (!IsA(jtnode, RangeTblRef)) return false;
...
static bool jointree_contains_lateral_outer_refs(PlannerInfo *root, Node *jtnode, bool restricted,
  ...
  if (IsA(jtnode, RangeTblRef))
    ...
    foreach (l, f->fromlist) {
      if (jointree_contains_lateral_outer_refs(root, lfirst(l), restricted, safe_upper_varnos)) return true;
    }

    /* Then check the top-level quals */
    if (restricted && !bms_is_subset(pull_varnos_of_level(root, f->quals, 1), safe_upper_varnos)) return true;
  } else if (IsA(jtnode, JoinExpr)) {
    JoinExpr *j = (JoinExpr *)jtnode;
    ...
    if (jointree_contains_lateral_outer_refs(root, j->larg, restricted, safe_upper_varnos)) return true;
    if (jointree_contains_lateral_outer_refs(root, j->rarg, restricted, safe_upper_varnos)) return true;

    /* Check the JOIN's qual clauses */
    if (restricted && !bms_is_subset(pull_varnos_of_level(root, j->quals, 1), safe_upper_varnos)) return true;
  } else
    elog(ERROR, "unrecognized node type: %d", (int)nodeTag(jtnode));
  return false;
}
...
static void perform_pullup_replace_vars(PlannerInfo *root, pullup_replace_vars_context *rvcontext,
  ...
  foreach (lc, root->append_rel_list) {
    ...
    if (appinfo == containing_appendrel) rvcontext->need_phvs = false;
...
static void replace_vars_in_jointree(Node *jtnode, pullup_replace_vars_context *context,
  ...
  if (IsA(jtnode, RangeTblRef)) {
    ...
    foreach (l, f->fromlist) replace_vars_in_jointree(lfirst(l), context, lowest_nulling_outer_join);
     ...
     */
    if (j->jointype == JOIN_FULL) context->need_phvs = true;
 ...
 */
static Node *pullup_replace_vars(Node *expr, pullup_replace_vars_context *context) {
  return replace_rte_variables(expr, context->varno, 0, pullup_replace_vars_callback, (void *)context,
...
static Node *pullup_replace_vars_callback(Var *var, replace_rte_variables_context *context) {
  ...
  if (rcon->need_phvs && varattno >= InvalidAttrNumber && varattno <= list_length(rcon->targetlist) &&
     ...
     */
    expandRTE(rcon->target_rte, var->varno, 0 /* not varlevelsup */, var->location, (var->vartype != RECORDOID),
              &colnames, &fields);
    ...
    if (rcon->need_phvs) {
      ...
      if (newnode && IsA(newnode, Var) && ((Var *)newnode)->varlevelsup == 0) {
         ...
         */
        if ((rcon->target_rte->lateral ? bms_overlap(pull_varnos(rcon->root, (Node *)newnode), rcon->relids)
      ...
      if (wrap) newnode = (Node *)make_placeholder_expr(rcon->root, (Expr *)newnode, bms_make_singleton(rcon->varno));
  ...
  if (var->varlevelsup > 0) IncrementVarSublevelsUp(newnode, var->varlevelsup, 0);
...
static Query *pullup_replace_vars_subquery(Query *query, pullup_replace_vars_context *context) {
  ...
  return (Query *)replace_rte_variables((Node *)query, context->varno, 1, pullup_replace_vars_callback, (void *)context,
                                        NULL);
}
...
void flatten_simple_union_all(PlannerInfo *root) {
  ...
  if (root->hasRecursion) return;

  /*
   * Recursively check the tree of set operations.  If not all UNION ALL
   * with identical column types, punt.
   */
  if (!is_simple_union_all_recurse((Node *)topop, parse, topop->colTypes)) return;
...
void reduce_outer_joins(PlannerInfo *root) {
  ...
  if (state == NULL || !state->contains_outer) elog(ERROR, "so where are the outer joins?");
...
static reduce_outer_joins_state *reduce_outer_joins_pass1(Node *jtnode) {
  ...
  if (IsA(jtnode, RangeTblRef)) {
    ...
    if (IS_OUTER_JOIN(j->jointype)) result->contains_outer = true;
...
static void reduce_outer_joins_pass2(Node *jtnode, reduce_outer_joins_state *state, PlannerInfo *root,
   ...
   */
  if (jtnode == NULL) elog(ERROR, "reached empty jointree");
  if (IsA(jtnode, RangeTblRef))
    ...
    forboth(l, f->fromlist, s, state->sub_states) {
      ...
      if (sub_state->contains_outer)
        reduce_outer_joins_pass2(lfirst(l), sub_state, root, pass_nonnullable_rels, pass_nonnullable_vars,
    ...
    switch (jointype) {
      case JOIN_INNER:
        break;
      case JOIN_LEFT:
        if (bms_overlap(nonnullable_rels, right_state->relids)) jointype = JOIN_INNER;
        break;
      case JOIN_RIGHT:
        if (bms_overlap(nonnullable_rels, left_state->relids)) jointype = JOIN_INNER;
        break;
      case JOIN_FULL:
        if (bms_overlap(nonnullable_rels, left_state->relids)) {
          ...
          if (bms_overlap(nonnullable_rels, right_state->relids)) jointype = JOIN_RIGHT;
        }
        break;
      case JOIN_SEMI:
      case JOIN_ANTI:
    ...
    if (jointype == JOIN_LEFT) {
      ...
      if (overlap != NIL && bms_overlap(pull_varnos(root, (Node *)overlap), right_state->relids)) jointype = JOIN_ANTI;
    }
    ...
    if (left_state->contains_outer || right_state->contains_outer) {
      ...
      if (jointype != JOIN_FULL) {
        ...
        if (!computed_local_nonnullable_vars) local_nonnullable_vars = find_nonnullable_vars(j->quals);
      ...
      if (left_state->contains_outer) {
        ...
        reduce_outer_joins_pass2(j->larg, left_state, root, pass_nonnullable_rels, pass_nonnullable_vars,
                                 pass_forced_null_vars);
      }
      ...
      if (right_state->contains_outer) {
        ...
        reduce_outer_joins_pass2(j->rarg, right_state, root, pass_nonnullable_rels, pass_nonnullable_vars,
                                 pass_forced_null_vars);
      }
...
static Node *remove_useless_results_recurse(PlannerInfo *root, Node *jtnode) {
  ...
  if (IsA(jtnode, RangeTblRef)) {
    ...
    foreach (cell, f->fromlist) {
       ...
       */
      if (list_length(f->fromlist) > 1 && (varno = get_result_relid(root, child)) != 0 &&
    ...
    if (result_relids) {
      ...
      while ((varno = bms_next_member(result_relids, varno)) >= 0) remove_result_refs(root, varno, (Node *)f);
    }
    ...
    switch (j->jointype) {
      ...
      case JOIN_RIGHT:
        ...
        if ((varno = get_result_relid(root, j->larg)) != 0 && (j->quals == NULL || !find_dependent_phvs(root, varno))) {
          remove_result_refs(root, varno, j->rarg);
          jtnode = j->rarg;
        }
        break;
...
static int get_result_relid(PlannerInfo *root, Node *jtnode) {
  ...
  if (!IsA(jtnode, RangeTblRef)) return 0;
  varno = ((RangeTblRef *)jtnode)->rtindex;
  if (rt_fetch(varno, root->parse->rtable)->rtekind != RTE_RESULT) return 0;
  return varno;
}
...
static bool find_dependent_phvs_walker(Node *node, find_dependent_phvs_context *context) {
  ...
  if (IsA(node, PlaceHolderVar)) {
    ...
    if (phv->phlevelsup == context->sublevels_up && bms_equal(context->relids, phv->phrels)) return true;
...
static bool find_dependent_phvs(PlannerInfo *root, int varno) {
  ...
  if (root->glob->lastPHId == 0) return false;
...
static bool find_dependent_phvs_in_jointree(PlannerInfo *root, Node *node, int varno) {
   ...
   */
  if (find_dependent_phvs_walker(node, &context)) return true;
  ...
  while ((relid = bms_next_member(subrelids, relid)) >= 0) {
    ...
    if (rte->lateral && range_table_entry_walker(rte, find_dependent_phvs_walker, (void *)&context, 0)) return true;
  }
...
static void fix_append_rel_relids(List *append_rel_list, int varno, Relids subrelids) {
  ...
  foreach (l, append_rel_list) {
    ...
    if (appinfo->child_relid == varno) {
      if (subvarno < 0) subvarno = bms_singleton_member(subrelids);
...
Relids get_relids_in_jointree(Node *jtnode, bool include_joins) {
  ...
  if (IsA(jtnode, RangeTblRef)) {
    ...
    if (include_joins && j->rtindex) result = bms_add_member(result, j->rtindex);
  } else
    elog(ERROR, "unrecognized node type: %d", (int)nodeTag(jtnode));
  return result;
}
...
Relids get_relids_for_join(Query *query, int joinrelid) {
  ...
  if (!jtnode) elog(ERROR, "could not find join node %d", joinrelid);
 ...
 */
static Node *find_jointree_node_for_rel(Node *jtnode, int relid) {
  if (jtnode == NULL) return NULL;
  if (IsA(jtnode, RangeTblRef)) {
    int varno = ((RangeTblRef *)jtnode)->rtindex;

    if (relid == varno) return jtnode;
  } else if (IsA(jtnode, FromExpr)) {
    FromExpr *f = (FromExpr *)jtnode;
    ListCell *l;
    ...
    foreach (l, f->fromlist) {
      ...
      if (jtnode) return jtnode;
    }
  } else if (IsA(jtnode, JoinExpr)) {
    JoinExpr *j = (JoinExpr *)jtnode;

    if (relid == j->rtindex) return jtnode;
