...
static void make_inh_translation_list(Relation oldrelation, Relation newrelation, Index newvarno,
  ...
  for (old_attno = 0; old_attno < oldnatts; old_attno++) {
    ...
    if (new_attno >= newnatts || (att = TupleDescAttr(new_tupdesc, new_attno))->attisdropped ||
      ...
      if (!HeapTupleIsValid(newtup))
        elog(ERROR, "could not find inherited attribute \"%s\" of relation \"%s\"", attname,
    ...
    if (attcollation != att->attcollation)
      elog(ERROR, "attribute \"%s\" of relation \"%s\" does not match parent's collation", attname,
           RelationGetRelationName(newrelation));
...
static Node *adjust_appendrel_attrs_mutator(Node *node, adjust_appendrel_attrs_context *context) {
  ...
  if (IsA(node, Var)) {
    ...
    if (var->varlevelsup != 0) return (Node *)var; /* no changes needed */
    ...
    if (appinfo) {
      ...
      if (var->varattno > 0) {
        ...
        if (var->varattno > list_length(appinfo->translated_vars))
          elog(ERROR, "attribute %d of relation \"%s\" does not exist", var->varattno,
      ...
      for (cnt = 0; cnt < nappinfos; cnt++) {
        if (bms_is_member(appinfos[cnt]->child_relid, leaf_result_relids)) {
          if (leaf_relid) elog(ERROR, "cannot translate to multiple leaf relids");
      ...
      if (leaf_relid) {
        RowIdentityVarInfo *ridinfo =
            (RowIdentityVarInfo *)list_nth(context->root->row_identity_vars, var->varattno - 1);
  ...
  if (IsA(node, PlaceHolderVar)) {
    ...
    if (phv->phlevelsup == 0) phv->phrels = adjust_child_relids(phv->phrels, context->nappinfos, context->appinfos);
    return (Node *)phv;
  }
...
Node *adjust_appendrel_attrs_multilevel(PlannerInfo *root, Node *node, RelOptInfo *childrel, RelOptInfo *parentrel) {
  ...
  if (childrel->parent != parentrel) {
    if (childrel->parent)
      node = adjust_appendrel_attrs_multilevel(root, node, childrel->parent, parentrel);
    else
...
Relids adjust_child_relids(Relids relids, int nappinfos, AppendRelInfo **appinfos) {
  ...
  for (cnt = 0; cnt < nappinfos; cnt++) {
    ...
    if (bms_is_member(appinfo->parent_relid, relids)) {
      ...
      if (!result) result = bms_copy(relids);
...
Relids adjust_child_relids_multilevel(PlannerInfo *root, Relids relids, RelOptInfo *childrel, RelOptInfo *parentrel) {
  ...
  if (childrel->parent != parentrel) {
    if (childrel->parent)
      relids = adjust_child_relids_multilevel(root, relids, childrel->parent, parentrel);
    else
      elog(ERROR, "childrel is not a child of parentrel");
  }
...
List *adjust_inherited_attnums(List *attnums, AppendRelInfo *context) {
  ...
  foreach (lc, attnums) {
    ...
    if (parentattno <= 0 || parentattno > list_length(context->translated_vars))
      elog(ERROR, "attribute %d of relation \"%s\" does not exist", parentattno, get_rel_name(context->parent_reloid));
...
List *adjust_inherited_attnums_multilevel(PlannerInfo *root, List *attnums, Index child_relid, Index top_parent_relid) {
  ...
  if (!appinfo) elog(ERROR, "child rel %d not found in append_rel_array", child_relid);

  /* Recurse if immediate parent is not the top parent. */
  if (appinfo->parent_relid != top_parent_relid)
    attnums = adjust_inherited_attnums_multilevel(root, attnums, appinfo->parent_relid, top_parent_relid);
...
void get_translated_update_targetlist(PlannerInfo *root, Index relid, List **processed_tlist, List **update_colnos) {
  ...
  if (relid == root->parse->resultRelation) {
    ...
    *processed_tlist = copyObject(root->processed_tlist);
    if (update_colnos) *update_colnos = copyObject(root->update_colnos);
  } else {
    Assert(bms_is_member(relid, root->all_result_relids));
    *processed_tlist =
        (List *)adjust_appendrel_attrs_multilevel(root, (Node *)root->processed_tlist, find_base_rel(root, relid),
                                                  find_base_rel(root, root->parse->resultRelation));
    if (update_colnos)
      *update_colnos =
          adjust_inherited_attnums_multilevel(root, root->update_colnos, relid, root->parse->resultRelation);
  }
}
...
AppendRelInfo **find_appinfos_by_relids(PlannerInfo *root, Relids relids, int *nappinfos) {
  ...
  *nappinfos = bms_num_members(relids);
  ...
  while ((i = bms_next_member(relids, i)) >= 0) {
    ...
    if (!appinfo) elog(ERROR, "child rel %d not found in append_rel_array", i);
...
void add_row_identity_var(PlannerInfo *root, Var *orig_var, Index rtindex, const char *rowid_name) {
  ...
  if (rtindex == root->parse->resultRelation) {
    tle = makeTargetEntry((Expr *)orig_var, list_length(root->processed_tlist) + 1, pstrdup(rowid_name), true);
  ...
  foreach (lc, root->row_identity_vars) {
    ...
    if (strcmp(rowid_name, ridinfo->rowidname) != 0) continue;
  ...
  ridinfo->rowidwidth = get_typavgwidth(exprType((Node *)rowid_var), exprTypmod((Node *)rowid_var));
  ...
  tle = makeTargetEntry((Expr *)rowid_var, list_length(root->processed_tlist) + 1, pstrdup(rowid_name), true);
  root->processed_tlist = lappend(root->processed_tlist, tle);
}
...
void add_row_identity_columns(PlannerInfo *root, Index rtindex, RangeTblEntry *target_rte, Relation target_relation) {
  ...
  if (commandType == CMD_MERGE || relkind == RELKIND_RELATION || relkind == RELKIND_MATVIEW ||
    ...
    if (commandType == CMD_UPDATE ||
        (target_relation->trigdesc &&
         (target_relation->trigdesc->trig_delete_after_row || target_relation->trigdesc->trig_delete_before_row))) {
      var = makeVar(rtindex, InvalidAttrNumber, RECORDOID, -1, InvalidOid, 0);
...
void distribute_row_identity_vars(PlannerInfo *root) {
   ...
   */
  if (parse->commandType != CMD_UPDATE && parse->commandType != CMD_DELETE && parse->commandType != CMD_MERGE) {
