...
int MainLoop(FILE *source) {
  ...
  while (successResult == EXIT_SUCCESS) {
    ...
    if (pset.cur_cmd_interactive) {
      ...
      if (query_buf->len == 0) prompt_status = PROMPT_READY;
      ...
      if (!line && ferror(source)) successResult = EXIT_FAILURE;
    }
    ...
    if (line == NULL) {
      if (pset.cur_cmd_interactive) {
        ...
        if (count_eof < pset.ignoreeof) {
          if (!pset.quiet) printf(_("Use \"\\q\" to leave %s.\n"), pset.progname);
          continue;
        }
    ...
    if (pset.lineno == 1 && !pset.cur_cmd_interactive && strncmp(line, "PGDMP", 5) == 0) {
      free(line);
      puts(
          _("The input is a PostgreSQL custom-format dump.\n"
    ...
    if (pset.cur_cmd_interactive) {
      ...
      if (found_help) {
        if (query_buf->len != 0)
          ...
          printf(
              _("Type:  \\copyright for distribution terms\n"
       ...
       */
      if (found_q && query_buf->len != 0 && prompt_status != PROMPT_READY && prompt_status != PROMPT_CONTINUE &&
    ...
    while (success || !die_on_error) {
      ...
      if (scan_result == PSCAN_EOL) pset.stmt_lineno++;
      ...
      if (scan_result == PSCAN_SEMICOLON || (scan_result == PSCAN_EOL && pset.singleline)) {
        ...
        if (slashCmdStatus == PSQL_CMD_SEND) {
          ...
          if (query_buf->len > 0 && query_buf->data[query_buf->len - 1] != '\n') appendPQExpBufferChar(query_buf, '\n');
      ...
      if (scan_result == PSCAN_INCOMPLETE || scan_result == PSCAN_EOL) break;
    }
     ...
     */
    if (pset.cur_cmd_interactive) {
      if (!line_saved_in_history) pg_append_history(line, history_buf);
      if (query_buf->len == 0) pg_send_history(history_buf);
    }
    ...
    if (!pset.cur_cmd_interactive) {
      if (!success && die_on_error) successResult = EXIT_USER;
...
#ifdef NOT_USED
    if (pset.cur_cmd_interactive) pg_send_history(history_buf);
#endif
