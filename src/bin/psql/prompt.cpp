  ...
  for (p = prompt_string; *p && strlen(destination) < sizeof(destination) - 1; p++) {
    ...
    if (esc) {
      switch (*p) {
          /* Current database */
        case '/':
          if (pset.db) strlcpy(buf, PQdb(pset.db), sizeof(buf));
          break;
        case '~':
          if (pset.db) {
            const char *var;
        ...
        case 'w':
          if (pset.db) memset(buf, ' ', Min(last_prompt1_width, sizeof(buf) - 1));
          break;
        ...
        case '>':
          if (pset.db && PQport(pset.db)) strlcpy(buf, PQport(pset.db), sizeof(buf));
          break;
          /* DB server user name */
        case 'n':
          if (pset.db) strlcpy(buf, session_username(), sizeof(buf));
          break;
          /* backend pid */
        case 'p':
          if (pset.db) {
            ...
            if (pid) snprintf(buf, sizeof(buf), "%d", pid);
          }
          break;
        ...
        case '6':
        case '7':
          *buf = (char)strtol(p, const_cast<char **>(&p), 8);
          --p;
          break;
        case 'R':
          switch (status) {
        ...
        case '`': {
          ...
          fd = popen(file, "r");
          if (fd) {
            if (fgets(buf, sizeof(buf), fd) == NULL) buf[0] = '\0';
            pclose(fd);
          }
        ...
        case ':': {
          ...
          val = pset.vars.GetVariable(name);
          if (val) strlcpy(buf, val, sizeof(buf));
          free(name);
          p += nameend + 1;
          break;
        }
    ...
    if (!esc) strlcat(destination, buf, sizeof(destination));
  }
  ...
  if (prompt_string == pset.prompt1) {
    ...
    while (*p) {
...
#endif
      {
        ...
        if (p + chlen > end) break; /* Invalid string */
