...
static int ExecQueryAndProcessResults(const char *query, double *elapsed_msec, bool *svpt_gone_p, bool is_watch,
...
bool setQFout(const char *fname) {
  ...
  if (!openQueryOutputFile(fname, &fout, &is_pipe)) return false;
...
void psql_setup_cancel_handler(void) { setup_cancel_handler(psql_cancel_callback); }
...
static void ClearOrSaveAllResults(void) {
  ...
  while ((result = PQgetResult(pset.db)) != NULL) ClearOrSaveResult(result);
}
...
PGresult *PSQLexec(const char *query) {
  ...
  if (pset.echo_hidden != PSQL_ECHO_HIDDEN_OFF) {
    ...
    if (pset.echo_hidden == PSQL_ECHO_HIDDEN_NOEXEC) return NULL;
  }
...
int PSQLexecWatch(const char *query, const printQueryOpt *opt, FILE *printQueryFout) {
  ...
  if (timing) PrintTiming(elapsed_msec);
...
static bool PrintQueryTuples(const PGresult *result, const printQueryOpt *opt, FILE *printQueryFout) {
  ...
  if (pset.gfname) {
    ...
    if (!openQueryOutputFile(pset.gfname, &fout, &is_pipe)) return false;
    if (is_pipe) disable_sigpipe_trap();
  ...
  for (r = 0; r < nrows; r++) {
    for (c = 0; c < ncolumns; c++) {
      if (!PQgetisnull(result, r, c)) {
        ...
        if (cancel_pressed) goto loop_exit;
        ...
        if (!SendQuery(query)) {
          ...
          success = false;
          if (pset.on_error_stop) goto loop_exit;
        }
      }
    }
  }
...
static bool HandleCopyResult(PGresult **resultp) {
  ...
  Assert(result_status == PGRES_COPY_OUT || result_status == PGRES_COPY_IN);
  ...
  if (result_status == PGRES_COPY_OUT) {
    ...
    success = handleCopyOut(pset.db, copystream, &copy_result) && (copystream != NULL);
    ...
    success = handleCopyIn(pset.db, copystream, PQbinaryTuples(*resultp), &copy_result);
  }
  ResetCancelConn();
 ...
 */
static bool PrintQueryResult(PGresult *result, bool last, bool is_watch, const printQueryOpt *opt,
                             FILE *printQueryFout) {
  bool success;
  ...
  switch (PQresultStatus(result)) {
    ...
    case PGRES_COMMAND_OK:
      if (last || pset.show_all_results) PrintQueryStatus(result, printQueryFout);
      success = true;
      break;
...
bool SendQuery(const char *query) {
  ...
  if (pset.singlestep) {
    ...
    if (fgets(buf, sizeof(buf), stdin) != NULL)
      if (buf[0] == 'x') goto sendquery_cleanup;
    if (cancel_pressed) goto sendquery_cleanup;
  ...
  if (on_error_rollback_savepoint) {
    ...
    switch (transaction_status) {
      ...
      case PQTRANS_INTRANS:
         ...
         */
        if (!svpt_gone) svptcmd = "RELEASE pg_psql_temporary_savepoint";
        break;
...
static bool DescribeQuery(const char *query, double *elapsed_msec) {
  ...
  *elapsed_msec = 0;
  ...
  if (timing) INSTR_TIME_SET_CURRENT(before);
  ...
  if (OK && result) {
    if (PQnfields(result) > 0) {
      ...
      for (i = 0; i < PQnfields(result); i++) {
        ...
        if (i > 0) appendPQExpBufferStr(&buf, ",");
        ...
        appendPQExpBuffer(&buf, "(%s, '%u'::pg_catalog.oid, %d)", escname, PQftype(result, i), PQfmod(result, i));
      ...
      if (OK && result) OK = PrintQueryResult(result, true, false, NULL, NULL);
...
static int ExecQueryAndProcessResults(const char *query, double *elapsed_msec, bool *svpt_gone_p, bool is_watch,
  ...
  while (result != NULL) {
    ...
    if (!AcceptResult(result, false)) {
       ...
       */
      if (result_status == PGRES_COPY_BOTH || result_status == PGRES_COPY_OUT || result_status == PGRES_COPY_IN)
      ...
      *svpt_gone_p = (strcmp(cmd, "COMMIT") == 0 || strcmp(cmd, "SAVEPOINT") == 0 || strcmp(cmd, "RELEASE") == 0 ||
    ...
    if (result != NULL) success &= PrintQueryResult(result, last, false, opt, printQueryFout);

    /* set variables on last result if all went well */
    if (!is_watch && last && success) SetResultVariables(result, true);
...
static bool ExecQueryUsingCursor(const char *query, double *elapsed_msec) {
  ...
  *elapsed_msec = 0;
  ...
  result = PQexec(pset.db, buf.data);
  OK = AcceptResult(result, true) && (PQresultStatus(result) == PGRES_COMMAND_OK);
  if (!OK) SetResultVariables(result, OK);
  ClearOrSaveResult(result);
  termPQExpBuffer(&buf);
  if (!OK) goto cleanup;
  ...
  for (;;) {
     ...
     */
    if (ntuples < fetch_count || cancel_pressed || flush_error || ferror(fout)) break;
  }
...
static bool command_no_begin(const char *query) {
   ...
   */
  wordlen = 0;
  while (isalpha((unsigned char)query[wordlen])) wordlen += PQmblenBounded(&query[wordlen], pset.encoding);
   ...
   */
  if (wordlen == 5 && pg_strncasecmp(query, "abort", 5) == 0) return true;
  if (wordlen == 5 && pg_strncasecmp(query, "begin", 5) == 0) return true;
  if (wordlen == 5 && pg_strncasecmp(query, "start", 5) == 0) return true;
  if (wordlen == 6 && pg_strncasecmp(query, "commit", 6) == 0) return true;
  if (wordlen == 3 && pg_strncasecmp(query, "end", 3) == 0) return true;
  if (wordlen == 8 && pg_strncasecmp(query, "rollback", 8) == 0) return true;
  if (wordlen == 7 && pg_strncasecmp(query, "prepare", 7) == 0) {
    ...
    if (wordlen == 11 && pg_strncasecmp(query, "transaction", 11) == 0) return true;
    return false;
  }
   ...
   * the backend.
   */
  if (wordlen == 6 && pg_strncasecmp(query, "vacuum", 6) == 0) return true;
  if (wordlen == 7 && pg_strncasecmp(query, "cluster", 7) == 0) {
    ...
    if (isalpha((unsigned char)query[0])) return false; /* has additional words */
  ...
  if (wordlen == 6 && pg_strncasecmp(query, "create", 6) == 0) {
    ...
    if (wordlen == 8 && pg_strncasecmp(query, "database", 8) == 0) return true;
    if (wordlen == 10 && pg_strncasecmp(query, "tablespace", 10) == 0) return true;
    ...
    if (wordlen == 5 && pg_strncasecmp(query, "index", 5) == 0) {
      ...
      if (wordlen == 12 && pg_strncasecmp(query, "concurrently", 12) == 0) return true;
    }
  ...
  if (wordlen == 5 && pg_strncasecmp(query, "alter", 5) == 0) {
    ...
    if (wordlen == 6 && pg_strncasecmp(query, "system", 6) == 0) return true;
  ...
  if (wordlen == 7 && pg_strncasecmp(query, "discard", 7) == 0) {
    ...
    if (wordlen == 3 && pg_strncasecmp(query, "all", 3) == 0) return true;
    return false;
  }
...
static bool is_select_command(const char *query) {
  ...
  if (wordlen == 6 && pg_strncasecmp(query, "select", 6) == 0) return true;

  if (wordlen == 6 && pg_strncasecmp(query, "values", 6) == 0) return true;
...
bool is_superuser(void) {
  ...
  if (!pset.db) return false;

  val = PQparameterStatus(pset.db, "is_superuser");

  if (val && strcmp(val, "on") == 0) return true;
...
static int uri_prefix_length(const char *connstr) {
  ...
  if (strncmp(connstr, uri_designator, sizeof(uri_designator) - 1) == 0) return sizeof(uri_designator) - 1;
