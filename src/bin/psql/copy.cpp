...
static struct copy_options *parse_slash_copy(const char *args) {
  ...
  if (pg_strcasecmp(token, "program") == 0) {
     ...
     */
    toklen = strlen(token);
    if (token[0] != '\'' || toklen < 2 || token[toklen - 1] != '\'') goto error;
  ...
  if (token) result->after_tofrom = pg_strdup(token);
...
bool do_copy(const char *args) {
  ...
  if (options->file && !options->program) canonicalize_path(options->file);
  ...
  if (options->after_tofrom) appendPQExpBufferStr(&query, options->after_tofrom);
...
bool handleCopyOut(PGconn *conn, FILE *copystream, PGresult **res) {
  ...
  for (;;) {
    ...
    if (ret < 0) break; /* done or server/connection error */
...
bool handleCopyIn(PGconn *conn, FILE *copystream, bool isbinary, PGresult **res) {
  ...
  if (isatty(fileno(copystream))) {
    ...
    if (!pset.quiet)
      puts(
          _("Enter data to be copied followed by a newline.\n"
