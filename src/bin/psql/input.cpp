...
void pg_send_history(PQExpBuffer history_buf) {
  ...
  if (useHistory && s[0]) {
    if (((pset.histcontrol & hctl_ignorespace) && s[0] == ' ') ||
        ((pset.histcontrol & hctl_ignoredups) && prev_hist && strcmp(s, prev_hist) == 0)) {
...
char *gets_fromFile(FILE *source) {
  ...
  for (;;) {
    ...
    if (result == NULL) {
      if (ferror(source)) {
        PSQL_LOG_ERROR("could not read from input file: %m");
        return NULL;
      }
      break;
    }
...
static void decode_history(void) {
  BEGIN_ITERATE_HISTORY(cur_hist);
  {
    ...
    /* some platforms declare HIST_ENTRY.line as const char * */
    for (cur_ptr = (char *)cur_hist->line; *cur_ptr; cur_ptr++) {
      if (*cur_ptr == NL_IN_HISTORY) *cur_ptr = '\n';
    }
  }
  END_ITERATE_HISTORY();
}
...
void initializeInput(int flags) {
  ...
  if (flags & 1) {
    ...
    if (histfile == NULL) {
      ...
      if (envhist != NULL && strlen(envhist) > 0) histfile = envhist;
    }

    if (histfile == NULL) {
      if (get_home_path(home)) psql_history = psprintf("%s/%s", home, PSQLHISTORY);
    } else {
...
static bool saveHistory(char *fname, int max_lines) {
  ...
  if (strcmp(fname, DEVNULL) != 0) {
    ...
    PSQL_LOG_ERROR("could not save history to file \"%s\": %m", fname);
  }
  return false;
}
#endif
