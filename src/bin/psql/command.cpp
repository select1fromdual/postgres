...
static backslashResult exec_command(const char *cmd, PsqlScanState scan_state, ConditionalStack cstack,
...
static bool exec_command_dfo(PsqlScanState scan_state, const char *cmd, const char *pattern, bool show_verbose,
                             bool show_system);
static backslashResult exec_command_edit(PsqlScanState scan_state, bool active_branch, PQExpBuffer query_buf,
                                         PQExpBuffer previous_buf);
static backslashResult exec_command_ef_ev(PsqlScanState scan_state, bool active_branch, PQExpBuffer query_buf,
                                          bool is_func);
...
static backslashResult process_command_g_options(char *first_option, PsqlScanState scan_state, bool active_branch,
                                                 const char *cmd);
...
static backslashResult exec_command_print(PsqlScanState scan_state, bool active_branch, PQExpBuffer query_buf,
                                          PQExpBuffer previous_buf);
...
static backslashResult exec_command_write(PsqlScanState scan_state, bool active_branch, const char *cmd,
                                          PQExpBuffer query_buf, PQExpBuffer previous_buf);
static backslashResult exec_command_watch(PsqlScanState scan_state, bool active_branch, PQExpBuffer query_buf,
                                          PQExpBuffer previous_buf);
 ...
 *
 * query_buf and previous_buf will be nullptr when executing a "-c"
 * command-line option.
 *
...
backslashResult HandleSlashCmds(PsqlScanState scan_state, ConditionalStack cstack, PQExpBuffer query_buf,
                                PQExpBuffer previous_buf) {
  backslashResult status;
  char *cmd;
  char *arg;

  Assert(scan_state != nullptr);
  ...
  if (status == PSQL_CMD_UNKNOWN) {
    PSQL_LOG_ERROR("invalid command \\%s", cmd);
    if (pset.cur_cmd_interactive) PSQL_LOG_ERROR("Try \\? for help.");
  ...
  if (status != PSQL_CMD_ERROR) {
    ...
    while ((arg = psql_scan_slash_option(scan_state, OT_NORMAL, nullptr, false))) {
      if (active_branch) PSQL_LOG_WARN("\\%s: extra argument \"{}\" ignored", cmd, arg);
      free(arg);
    }
    conditional_stack_pop(cstack);
  } else {
    /* silently throw away rest of line after an erroneous command */
    while ((arg = psql_scan_slash_option(scan_state, OT_WHOLE_LINE, nullptr, false))) free(arg);
  }
...
static backslashResult exec_command(const char *cmd, PsqlScanState scan_state, ConditionalStack cstack,
  ...
  if (pset.cur_cmd_interactive && !active_branch && !is_branching_command(cmd)) {
    PSQL_LOG_WARN("\\%s command ignored; use \\endif or Ctrl-C to exit current \\if block", cmd);
  }
   ...
   */
  if (status == PSQL_CMD_SEND) (void)copy_previous_query(query_buf, previous_buf);
...
static backslashResult exec_command_C(PsqlScanState scan_state, bool active_branch) {
  ...
  if (active_branch) {
    char *opt = psql_scan_slash_option(scan_state, OT_NORMAL, nullptr, true);
...
static backslashResult exec_command_connect(PsqlScanState scan_state, bool active_branch) {
  ...
  if (active_branch) {
    ...
    if (opt1 != nullptr && strncmp(opt1, prefix, sizeof(prefix) - 1) == 0) {
      bool on_off;
...
static backslashResult exec_command_cd(PsqlScanState scan_state, bool active_branch, const char *cmd) {
  ...
  if (active_branch) {
    ...
    if (opt)
      ...
      if (dir == nullptr || dir[0] == '\0') {
        ...
        if (pw)
          ...
          PSQL_LOG_ERROR("could not get home directory for user ID {}: {}", (long)user_id,
    ...
    if (success && chdir(dir) < 0) {
      PSQL_LOG_ERROR("\\%s: could not change directory to \"%s\": %m", cmd, dir);
      success = false;
    }
...
static backslashResult exec_command_copy(PsqlScanState scan_state, bool active_branch) {
  ...
  if (active_branch) {
    char *opt = psql_scan_slash_option(scan_state, OT_WHOLE_LINE, nullptr, false);
 ...
 */
static backslashResult exec_command_copyright(PsqlScanState scan_state, bool active_branch) {
  if (active_branch) print_copyright();
...
static backslashResult exec_command_crosstabview(PsqlScanState scan_state, bool active_branch) {
  ...
  if (active_branch) {
    ...
    for (i = 0; i < lengthof(pset.ctv_args); i++)
      pset.ctv_args[i] = psql_scan_slash_option(scan_state, OT_NORMAL, nullptr, true);
...
static backslashResult exec_command_d(PsqlScanState scan_state, bool active_branch, const char *cmd) {
  ...
  if (active_branch) {
    ...
    pattern = psql_scan_slash_option(scan_state, OT_NORMAL, nullptr, true);
    ...
    switch (cmd[1]) {
      ...
      case 'S':
        if (pattern)
          ...
          success = listTables("tvmsE", nullptr, show_verbose, show_system);
        break;
      case 'A': {
        char *pattern2 = nullptr;

        if (pattern && cmd[2] != '\0' && cmd[2] != '+')
          pattern2 = psql_scan_slash_option(scan_state, OT_NORMAL, nullptr, true);
      ...
      case 'r':
        if (cmd[2] == 'd' && cmd[3] == 's') {
          ...
          if (pattern) pattern2 = psql_scan_slash_option(scan_state, OT_NORMAL, nullptr, true);
  ...
  if (!success) status = PSQL_CMD_ERROR;
...
static bool exec_command_dfo(PsqlScanState scan_state, const char *cmd, const char *pattern, bool show_verbose,
  ...
  if (pattern) /* otherwise it was just \df or \do */
  {
    ...
    while ((ap = psql_scan_slash_option(scan_state, OT_NORMAL, nullptr, true)) != nullptr) {
      arg_patterns[num_arg_patterns++] = ap;
      if (num_arg_patterns >= FUNC_MAX_ARGS) break; /* protect limited-size array */
    }
  }

  if (cmd[1] == 'f')
    success = describeFunctions(&cmd[2], pattern, arg_patterns, num_arg_patterns, show_verbose, show_system);
  else
    success = describeOperators(pattern, arg_patterns, num_arg_patterns, show_verbose, show_system);

  while (--num_arg_patterns >= 0) free(arg_patterns[num_arg_patterns]);
...
static backslashResult exec_command_edit(PsqlScanState scan_state, bool active_branch, PQExpBuffer query_buf,
  ...
  if (active_branch) {
    if (!query_buf) {
      PSQL_LOG_ERROR("no query buffer");
      status = PSQL_CMD_ERROR;
    } else {
      char *fname;
      char *ln = nullptr;
      ...
      fname = psql_scan_slash_option(scan_state, OT_NORMAL, nullptr, true);
      if (fname) {
        /* try to get separate lineno arg */
        ln = psql_scan_slash_option(scan_state, OT_NORMAL, nullptr, true);
        if (ln == nullptr) {
      ...
      if (ln) {
        lineno = atoi(ln);
        if (lineno < 1) {
          PSQL_LOG_ERROR("invalid line number: %s", ln);
          status = PSQL_CMD_ERROR;
        }
      }
      if (status != PSQL_CMD_ERROR) {
        ...
        if (do_edit(fname, query_buf, lineno, discard_on_quit, nullptr))
...
static backslashResult exec_command_ef_ev(PsqlScanState scan_state, bool active_branch, PQExpBuffer query_buf,
  ...
  if (active_branch) {
    char *obj_desc = psql_scan_slash_option(scan_state, OT_WHOLE_LINE, nullptr, true);
    int lineno = -1;
    ...
    if (status != PSQL_CMD_ERROR) {
      ...
      if (!do_edit(nullptr, query_buf, lineno, true, &edited))
        status = PSQL_CMD_ERROR;
      else if (!edited)
        puts(_("No changes"));
      else
...
static backslashResult exec_command_encoding(PsqlScanState scan_state, bool active_branch) {
  if (active_branch) {
    char *encoding = psql_scan_slash_option(scan_state, OT_NORMAL, nullptr, false);
    ...
    if (!encoding) {
      ...
      if (PQsetClientEncoding(pset.db, encoding) == -1)
        PSQL_LOG_ERROR("%s: invalid encoding name or conversion procedure not found", encoding);
      else {
...
static backslashResult exec_command_f(PsqlScanState scan_state, bool active_branch) {
  ...
  if (active_branch) {
    char *fname = psql_scan_slash_option(scan_state, OT_NORMAL, nullptr, false);
...
static backslashResult exec_command_g(PsqlScanState scan_state, bool active_branch, const char *cmd) {
   ...
   */
  fname = psql_scan_slash_option(scan_state, OT_FILEPIPE, nullptr, false);
  ...
  if (status == PSQL_CMD_SKIP_LINE && active_branch) {
    ...
    if (strcmp(cmd, "gx") == 0) {
      ...
      if (pset.gsavepopt == nullptr) pset.gsavepopt = savePsetInfo(&pset.popt);
...
static backslashResult process_command_g_options(char *first_option, PsqlScanState scan_state, bool active_branch,
  ...
  do {
    ...
    if (first_option)
      ...
      option = psql_scan_slash_option(scan_state, OT_NORMAL, nullptr, false);
      if (!option) {
        if (active_branch) {
          PSQL_LOG_ERROR("\\%s: missing right parenthesis", cmd);
          success = false;
        }
        break;
      }
...
static backslashResult exec_command_getenv(PsqlScanState scan_state, bool active_branch, const char *cmd) {
  ...
  if (active_branch) {
    char *myvar = psql_scan_slash_option(scan_state, OT_NORMAL, nullptr, false);
    char *envvar = psql_scan_slash_option(scan_state, OT_NORMAL, nullptr, false);

    if (!myvar || !envvar) {
      PSQL_LOG_ERROR("\\%s: missing required argument", cmd);
      success = false;
    } else {
      char *envval = getenv(envvar);

      if (envval && !pset.vars.SetVariable(myvar, envval)) success = false;
    }
    free(myvar);
    free(envvar);
  } else
...
static backslashResult exec_command_gset(PsqlScanState scan_state, bool active_branch) {
  ...
  if (active_branch) {
    char *prefix = psql_scan_slash_option(scan_state, OT_NORMAL, nullptr, false);

    if (prefix)
      pset.gset_prefix = prefix;
    else {
      /* we must set a non-nullptr prefix to trigger storing */
...
static backslashResult exec_command_help(PsqlScanState scan_state, bool active_branch) {
  if (active_branch) {
    ...
    if (opt) {
      len = strlen(opt);
      while (len > 0 && (isspace((unsigned char)opt[len - 1]) || opt[len - 1] == ';')) opt[--len] = '\0';
    }
...
static backslashResult exec_command_include(PsqlScanState scan_state, bool active_branch, const char *cmd) {
  ...
  if (active_branch) {
    char *fname = psql_scan_slash_option(scan_state, OT_NORMAL, nullptr, true);
...
static backslashResult exec_command_if(PsqlScanState scan_state, ConditionalStack cstack, PQExpBuffer query_buf) {
  if (conditional_active(cstack)) {
     ...
     */
    if (!is_true_boolean_expression(scan_state, "\\if expression")) conditional_stack_poke(cstack, IFSTATE_FALSE);
  } else {
    /*
...
static backslashResult exec_command_elif(PsqlScanState scan_state, ConditionalStack cstack, PQExpBuffer query_buf) {
  ...
  switch (conditional_stack_peek(cstack)) {
    ...
    case IFSTATE_FALSE:
      ...
      if (!is_true_boolean_expression(scan_state, "\\elif expression")) conditional_stack_poke(cstack, IFSTATE_FALSE);
      break;
    ...
    case IFSTATE_ELSE_FALSE:
      PSQL_LOG_ERROR("\\elif: cannot occur after \\else");
      success = false;
      break;
    case IFSTATE_NONE:
      /* no \if to elif from */
      PSQL_LOG_ERROR("\\elif: no matching \\if");
      success = false;
      break;
  }
...
static backslashResult exec_command_else(PsqlScanState scan_state, ConditionalStack cstack, PQExpBuffer query_buf) {
  ...
  switch (conditional_stack_peek(cstack)) {
    ...
    case IFSTATE_ELSE_TRUE:
    case IFSTATE_ELSE_FALSE:
      PSQL_LOG_ERROR("\\else: cannot occur after \\else");
      success = false;
      break;
    case IFSTATE_NONE:
      /* no \if to else from */
      PSQL_LOG_ERROR("\\else: no matching \\if");
      success = false;
      break;
  }
...
static backslashResult exec_command_endif(PsqlScanState scan_state, ConditionalStack cstack, PQExpBuffer query_buf) {
  ...
  switch (conditional_stack_peek(cstack)) {
    ...
    case IFSTATE_NONE:
      /* no \if to end */
      PSQL_LOG_ERROR("\\endif: no matching \\if");
      success = false;
      break;
  }
...
static backslashResult exec_command_lo(PsqlScanState scan_state, bool active_branch, const char *cmd) {
  ...
  if (active_branch) {
    ...
    opt1 = psql_scan_slash_option(scan_state, OT_NORMAL, nullptr, true);
    opt2 = psql_scan_slash_option(scan_state, OT_NORMAL, nullptr, true);
...
static backslashResult exec_command_out(PsqlScanState scan_state, bool active_branch) {
  ...
  if (active_branch) {
    char *fname = psql_scan_slash_option(scan_state, OT_FILEPIPE, nullptr, true);
...
static backslashResult exec_command_print(PsqlScanState scan_state, bool active_branch, PQExpBuffer query_buf,
  ...
  if (active_branch) {
     ...
     * Also, beware of possibility that buffer pointers are nullptr.
     */
...
static backslashResult exec_command_password(PsqlScanState scan_state, bool active_branch) {
  ...
  if (active_branch) {
    char *user = psql_scan_slash_option(scan_state, OT_SQLID, nullptr, true);
    char *pw1 = nullptr;
    char *pw2 = nullptr;
    PQExpBufferData buf;
    ...
    if (!prompt_ctx.canceled) pw2 = simple_prompt_extended("Enter it again: ", false, &prompt_ctx);

    if (prompt_ctx.canceled) {
      /* fail silently */
      success = false;
    } else if (strcmp(pw1, pw2) != 0) {
      PSQL_LOG_ERROR("Passwords didn't match.");
      success = false;
    } else {
      char *encrypted_password;

      encrypted_password = PQencryptPasswordConn(pset.db, pw1, user, nullptr);
...
static backslashResult exec_command_prompt(PsqlScanState scan_state, bool active_branch, const char *cmd) {
  ...
  if (active_branch) {
    char *opt, *prompt_text = nullptr;
    char *arg1, *arg2;

    arg1 = psql_scan_slash_option(scan_state, OT_NORMAL, nullptr, false);
    arg2 = psql_scan_slash_option(scan_state, OT_NORMAL, nullptr, false);
    ...
    if (!arg1) {
      ...
      if (!pset.inputfile) {
        ...
        if (!result) {
          PSQL_LOG_ERROR("\\%s: could not read value for variable", cmd);
          success = false;
        }
      }

      if (prompt_ctx.canceled || (result && !pset.vars.SetVariable(opt, result))) success = false;
...
static backslashResult exec_command_pset(PsqlScanState scan_state, bool active_branch) {
  ...
  if (active_branch) {
    char *opt0 = psql_scan_slash_option(scan_state, OT_NORMAL, nullptr, false);
    char *opt1 = psql_scan_slash_option(scan_state, OT_NORMAL, nullptr, false);

    if (!opt0) {
      /* list all variables */

      int i;
      static const char *const my_list[] = {"border",
                                            "columns",
                                            "csv_fieldsep",
                                            "expanded",
                                            "fieldsep",
...
static backslashResult exec_command_quit(PsqlScanState scan_state, bool active_branch) {
  ...
  if (active_branch) status = PSQL_CMD_TERMINATE;
...
static backslashResult exec_command_reset(PsqlScanState scan_state, bool active_branch, PQExpBuffer query_buf) {
  if (active_branch) {
    ...
    if (!pset.quiet) puts(_("Query buffer reset (cleared)."));
  }
...
static backslashResult exec_command_s(PsqlScanState scan_state, bool active_branch) {
  ...
  if (active_branch) {
    ...
    if (success && !pset.quiet && fname) printf(_("Wrote history to file \"%s\".\n"), fname);
    if (!fname) putchar('\n');
    free(fname);
  } else
    ignore_slash_options(scan_state);
...
static backslashResult exec_command_set(PsqlScanState scan_state, bool active_branch) {
  ...
  if (active_branch) {
    ...
    if (!opt0) {
      ...
      opt = psql_scan_slash_option(scan_state, OT_NORMAL, nullptr, false);
      newval = pg_strdup(opt ? opt : "");
      free(opt);

      while ((opt = psql_scan_slash_option(scan_state, OT_NORMAL, nullptr, false))) {
        newval = (char *)realloc(newval, strlen(newval) + strlen(opt) + 1);
        strcat(newval, opt);
        free(opt);
      }

      if (!pset.vars.SetVariable(opt0, newval)) success = false;
...
static backslashResult exec_command_setenv(PsqlScanState scan_state, bool active_branch, const char *cmd) {
  ...
  if (active_branch) {
    char *envvar = psql_scan_slash_option(scan_state, OT_NORMAL, nullptr, false);
    char *envval = psql_scan_slash_option(scan_state, OT_NORMAL, nullptr, false);

    if (!envvar) {
      PSQL_LOG_ERROR("\\%s: missing required argument", cmd);
      success = false;
    } else if (strchr(envvar, '=') != nullptr) {
      PSQL_LOG_ERROR("\\%s: environment variable name must not contain \"=\"", cmd);
      success = false;
    } else if (!envval) {
...
static backslashResult exec_command_sf_sv(PsqlScanState scan_state, bool active_branch, const char *cmd, bool is_func) {
  ...
  if (active_branch) {
    bool show_linenumbers = (strchr(cmd, '+') != nullptr);
    PQExpBuffer buf;
    char *obj_desc;
    Oid obj_oid = InvalidOid;
    EditableObjectType eot = is_func ? EditableFunction : EditableView;
    ...
    obj_desc = psql_scan_slash_option(scan_state, OT_WHOLE_LINE, nullptr, true);
    if (!obj_desc) {
      if (is_func)
        PSQL_LOG_ERROR("function name is required");
      else
        PSQL_LOG_ERROR("view name is required");
      status = PSQL_CMD_ERROR;
    } else if (!lookup_object_oid(eot, obj_desc, &obj_oid)) {
      /* error already reported */
      status = PSQL_CMD_ERROR;
      ...
      if (show_linenumbers) {
         ...
         */
        print_with_linenumbers(output, buf->data, is_func ? "AS " : nullptr);
      } else {
        /* just send the definition to output */
        fputs(buf->data, output);
      }

      if (is_pager) ClosePager(output);
    }
...
static backslashResult exec_command_T(PsqlScanState scan_state, bool active_branch) {
  ...
  if (active_branch) {
    char *value = psql_scan_slash_option(scan_state, OT_NORMAL, nullptr, false);
...
static backslashResult exec_command_timing(PsqlScanState scan_state, bool active_branch) {
  ...
  if (active_branch) {
    char *opt = psql_scan_slash_option(scan_state, OT_NORMAL, nullptr, false);
...
static backslashResult exec_command_unset(PsqlScanState scan_state, bool active_branch, const char *cmd) {
  ...
  if (active_branch) {
    ...
    if (!opt) {
      PSQL_LOG_ERROR("\\%s: missing required argument", cmd);
      success = false;
    } else if (!pset.vars.SetVariable(opt, nullptr))
      success = false;
...
static backslashResult exec_command_write(PsqlScanState scan_state, bool active_branch, const char *cmd,
  ...
  if (active_branch) {
    ...
    if (is_pipe) restore_sigpipe_trap();
...
static backslashResult exec_command_z(PsqlScanState scan_state, bool active_branch) {
  ...
  if (active_branch) {
    char *pattern = psql_scan_slash_option(scan_state, OT_NORMAL, nullptr, true);
 ...
 *
 * Returns a malloc'd string, or nullptr if no/empty argument.
 */
static char *read_connect_arg(PsqlScanState scan_state) {
  char *result;
  char quote;
...
static PQExpBuffer gather_boolean_expression(PsqlScanState scan_state) {
  ...
  while ((value = psql_scan_slash_option(scan_state, OT_NORMAL, nullptr, false)) != nullptr) {
    /* add spaces between tokens */
    if (num_options > 0) appendPQExpBufferChar(exp_buf, ' ');
...
static void ignore_slash_options(PsqlScanState scan_state) {
  ...
  while ((arg = psql_scan_slash_option(scan_state, OT_NORMAL, nullptr, false)) != nullptr) free(arg);
}
 ...
 */
static void ignore_slash_filepipe(PsqlScanState scan_state) {
  char *arg = psql_scan_slash_option(scan_state, OT_FILEPIPE, nullptr, false);
 ...
 */
static void ignore_slash_whole_line(PsqlScanState scan_state) {
  char *arg = psql_scan_slash_option(scan_state, OT_WHOLE_LINE, nullptr, false);
 ...
 */
static void save_query_text_state(PsqlScanState scan_state, ConditionalStack cstack, PQExpBuffer query_buf) {
  if (query_buf) conditional_stack_set_query_len(cstack, query_buf->len);
 ...
 * case of query_buf == nullptr (and do nothing).
 *
...
static char *prompt_for_password(const char *username, bool *canceled) {
  ...
  if (canceled) *canceled = prompt_ctx.canceled;
...
static bool param_is_newly_set(const char *old_val, const char *new_val) {
  ...
  if (old_val == nullptr || strcmp(old_val, new_val) != 0) return true;
 ...
 */
static bool do_connect(enum trivalue reuse_previous_specification, char *dbname, char *user, char *host, char *port) {
  PGconn *o_conn = pset.db, *n_conn = nullptr;
  ...
  if (has_connection_string && (user || host || port)) {
    PSQL_LOG_ERROR("Do not give user, host, or port separately when using a connection string");
    return false;
  }
  ...
  if (reuse_previous) {
    if (o_conn)
      ...
      PSQL_LOG_ERROR("No database connection exists to re-use parameters from");
      return false;
    }
  } else
    ...
    if (has_connection_string) {
      ...
      if (replcinfo) {
        ...
        for (ci = cinfo, replci = replcinfo; ci->keyword && replci->keyword; ci++, replci++) {
          ...
          if (replci->val) {
             ...
             */
            if (replci->val == nullptr || strcmp(ci->val, replci->val) != 0) {
              if (strcmp(replci->keyword, "user") == 0 || strcmp(replci->keyword, "host") == 0 ||
                  strcmp(replci->keyword, "hostaddr") == 0 || strcmp(replci->keyword, "port") == 0)
                keep_password = false;
            }
            /* Also note whether connstring contains a password. */
            if (strcmp(replci->keyword, "password") == 0) have_password = true;
        ...
        Assert(ci->keyword == nullptr && replci->keyword == nullptr);
         ...
         */
        if (have_password) keep_password = true;
      ...
      for (ci = cinfo; ci->keyword; ci++) {
        if (user && strcmp(ci->keyword, "user") == 0) {
          if (!(ci->val && strcmp(user, ci->val) == 0)) keep_password = false;
          ...
          if (!(ci->val && strcmp(port, ci->val) == 0)) keep_password = false;
        }
      }
  ...
  if (pset.getPassword == TRI_YES && success) {
     ...
     */
    password = prompt_for_password(has_connection_string ? nullptr : user, &canceled);
    success = !canceled;
  }
  ...
  while (success) {
    ...
    if (PQstatus(n_conn) == CONNECTION_OK) break;
    ...
    /*
     * We'll report the error below ... unless n_conn is nullptr, indicating
     * that libpq didn't have enough memory to make a PGconn.
     */
    if (n_conn == nullptr) PSQL_LOG_ERROR("out of memory");
  ...
  if (!success) {
    ...
    if (pset.cur_cmd_interactive) {
      if (n_conn) {
        PSQL_LOG_INFO("%s", PQerrorMessage(n_conn));
        PQfinish(n_conn);
      }

      /* pset.db is left unmodified */
      if (o_conn) PSQL_LOG_INFO("Previous connection kept");
    } else {
      if (n_conn) {
        PSQL_LOG_ERROR("\\connect: %s", PQerrorMessage(n_conn));
        PQfinish(n_conn);
      }
   ...
   */
  PQsetNoticeProcessor(n_conn, NoticeProcessor, nullptr);
  pset.db = n_conn;
  SyncVariables();
  ...
  if (!pset.quiet) {
    if (!o_conn || param_is_newly_set(PQhost(o_conn), PQhost(pset.db)) ||
      ...
      if (is_unixsock_path(host)) {
        ...
        if (hostaddr && *hostaddr && strcmp(host, hostaddr) != 0)
          printf(_("You are now connected to database \"%s\" as user \"%s\" on host \"%s\" (address \"%s\") at port "
                   "\"%s\".\n"),
  ...
  if (!pset.quiet && !pset.notty) {
    ...
    if (pset.sversion / 100 > client_ver / 100 || pset.sversion < 90200)
      printf(_("WARNING: %s major version %s, server major version %s.\n"
             ...
             pset.progname, formatPGVersionNumber(client_ver, false, cverbuf, sizeof(cverbuf)),
             formatPGVersionNumber(pset.sversion, false, sverbuf, sizeof(sverbuf)));

#ifdef WIN32
    if (in_startup) checkWin32Codepage();
#endif
    printSSLInfo();
    printGSSInfo();
  }
...
static void printSSLInfo(void) {
  ...
  if (!PQsslInUse(pset.db)) return; /* no SSL */
  ...
  printf(_("SSL connection (protocol: %s, cipher: %s, compression: %s)\n"), protocol ? protocol : _("unknown"),
         cipher ? cipher : _("unknown"), (compression && strcmp(compression, "off") != 0) ? _("on") : _("off"));
}
 ...
 */
static void printGSSInfo(void) {
  if (!PQgssEncInUse(pset.db)) return; /* no GSSAPI encryption in use */
...
void SyncVariables(void) {
  ...
  pset.vars.SetVariable("DBNAME", PQdb(pset.db));
  pset.vars.SetVariable("USER", PQuser(pset.db));
  pset.vars.SetVariable("HOST", PQhost(pset.db));
  pset.vars.SetVariable("PORT", PQport(pset.db));
  pset.vars.SetVariable("ENCODING", pg_encoding_to_char(pset.encoding));
  ...
  pset.vars.SetVariable("SERVER_VERSION_NAME", server_version);

  snprintf(vbuf, sizeof(vbuf), "%d", pset.sversion);
  pset.vars.SetVariable("SERVER_VERSION_NUM", vbuf);
...
void UnsyncVariables(void) {
  pset.vars.SetVariable("DBNAME", nullptr);
  pset.vars.SetVariable("USER", nullptr);
  pset.vars.SetVariable("HOST", nullptr);
  pset.vars.SetVariable("PORT", nullptr);
  pset.vars.SetVariable("ENCODING", nullptr);
  pset.vars.SetVariable("SERVER_VERSION_NAME", nullptr);
  pset.vars.SetVariable("SERVER_VERSION_NUM", nullptr);
}
...
static bool editFile(const char *fname, int lineno) {
  ...
  const char *editor_lineno_arg = nullptr;
  char *sys;
  int result;
  ...
  if (lineno > 0) {
    ...
    if (!editor_lineno_arg) editor_lineno_arg = DEFAULT_EDITOR_LINENUMBER_ARG;
#endif
    if (!editor_lineno_arg) {
      PSQL_LOG_ERROR("environment variable PSQL_EDITOR_LINENUMBER_ARG must be set to specify a line number");
      return false;
    }
  }
  ...
  result = system(sys);
  if (result == -1)
    PSQL_LOG_ERROR("could not start editor \"%s\"", editorName);
  else if (result == 127)
    PSQL_LOG_ERROR("could not start /bin/sh");
  free(sys);
 ...
 * "discard_on_quit" = true.
 *
 * If "edited" isn't nullptr, *edited will be set to true if the query buffer
...
static bool do_edit(const char *filename_arg, PQExpBuffer query_buf, int lineno, bool discard_on_quit, bool *edited) {
  ...
  if (filename_arg)
    ...
    if (ret == 0 || ret > MAXPGPATH) {
      PSQL_LOG_ERROR("could not locate temporary directory: %s", !ret ? strerror(errno) : "");
      return false;
    }
#endif
...
#else
    snprintf(fnametmp, sizeof(fnametmp), "%s%spsql.edit.%d.sql", tmpdir, "" /* trailing separator already present */,
             (int)getpid());
#endif
    ...
    if (fd != -1) stream = fdopen(fd, "w");

    if (fd == -1 || !stream) {
      PSQL_LOG_ERROR("could not open temporary file \"%s\": %m", fname);
      error = true;
    } else {
      ...
      if (fwrite(query_buf->data, 1, ql, stream) != ql) {
        ...
        if (fclose(stream) != 0) PSQL_LOG_ERROR("%s: %m", fname);

        if (remove(fname) != 0) PSQL_LOG_ERROR("%s: %m", fname);
         ...
         */
        ut.modtime = ut.actime = time(nullptr) - 2;
        (void)utime(fname, &ut);
      }
    }
  }
  ...
  /* call editor */
  if (!error) error = !editFile(fname, lineno);
  ...
  if (!error && (before.st_size != after.st_size || before.st_mtime != after.st_mtime)) {
    ...
    if (!stream) {
      ...
      resetPQExpBuffer(query_buf);
      while (fgets(line, sizeof(line), stream) != nullptr) appendPQExpBufferStr(query_buf, line);
     ...
     * the query buffer.
     */
    if (discard_on_quit) resetPQExpBuffer(query_buf);
  }
 ...
 * do_pset
 *
 * Performs the assignment "param = value", where value could be nullptr;
...
bool do_pset(const char *param, const char *value, printQueryOpt *popt, bool quiet) {
  ...
  if (strcmp(param, "format") == 0) {
    static const struct fmt {
      ...
      enum printFormat number;
                   ...
                   {"aligned", PRINT_ALIGNED},     {"asciidoc", PRINT_ASCIIDOC}, {"csv", PRINT_CSV},
                   {"html", PRINT_HTML},           {"latex", PRINT_LATEX},       {"troff-ms", PRINT_TROFF_MS},
                   {"unaligned", PRINT_UNALIGNED}, {"wrapped", PRINT_WRAPPED}};
    ...
    if (!value)
      ...
      for (int i = 0; i < lengthof(formats); i++) {
        if (pg_strncasecmp(formats[i].name, value, vallen) == 0) {
          if (match_pos < 0)
            ...
            PSQL_LOG_ERROR("\\pset: ambiguous abbreviation \"%s\" matches both \"%s\" and \"%s\"", value,
      ...
      if (match_pos >= 0)
        ...
        PSQL_LOG_ERROR(
            "\\pset: allowed formats are aligned, asciidoc, csv, html, latex, latex-longtable, troff-ms, unaligned, "
            "wrapped");
        return false;
      }
    }
  ...
  /* set table line style */
    ...
    if (!value)
      ...
      PSQL_LOG_ERROR("\\pset: allowed line styles are ascii, old-ascii, unicode");
      return false;
    }
  }
  ...
  /* set unicode border line style */
    ...
    if (!value)
      ...
      PSQL_LOG_ERROR("\\pset: allowed Unicode border line styles are single, double");
      return false;
    }
  }
  ...
  /* set unicode column line style */
    ...
    if (!value)
      ...
      PSQL_LOG_ERROR("\\pset: allowed Unicode column line styles are single, double");
      return false;
    }
  }
  ...
  /* set unicode header line style */
    ...
    if (!value)
      ...
      PSQL_LOG_ERROR("\\pset: allowed Unicode header line styles are single, double");
      return false;
    }
  }
  ...
  /* set border style/width */
    ...
    if (value) popt->topt.border = atoi(value);
  }
  ...
  /* set expanded/vertical mode */
    ...
    if (value && pg_strcasecmp(value, "auto") == 0)
      ...
      if (ParseVariableBool(value, nullptr, &on_off))
  ...
  /* header line width in expanded mode */
    ...
    if (!value)
      ...
      if (popt->topt.expanded_header_exact_width == 0) {
        PSQL_LOG_ERROR(
            "\\pset: allowed xheader_width values are full (default), column, page, or a number specifying the exact "
            "width.");
        return false;
      }
    }
  ...
  /* field separator for CSV format */
    ...
    if (value) {
      ...
      if (strlen(value) != 1) {
        PSQL_LOG_ERROR("\\pset: csv_fieldsep must be a single one-byte character");
        return false;
      }
      if (value[0] == '"' || value[0] == '\n' || value[0] == '\r') {
        PSQL_LOG_ERROR("\\pset: csv_fieldsep cannot be a double quote, a newline, or a carriage return");
        return false;
      }
  ...
  /* field separator for unaligned text */
    ...
    free(popt->topt.fieldSep.separator);
    popt->topt.fieldSep.separator = nullptr;
  ...
  /* record separator for unaligned text */
    ...
    free(popt->topt.recordSep.separator);
    popt->topt.recordSep.separator = nullptr;
  ...
  /* set minimum lines for pager use */
    ...
    if (value) popt->topt.pager_min_lines = atoi(value);
  }
  ...
  /* set border style/width */
    ...
    if (value) popt->topt.columns = atoi(value);
  } else {
    PSQL_LOG_ERROR("\\pset: unknown option: %s", param);
    return false;
  }

  if (!quiet) printPsetInfo(param, &pset.popt);
...
static bool printPsetInfo(const char *param, printQueryOpt *popt) {
  ...
  if (strcmp(param, "border") == 0) printf(_("Border style is %d.\n"), popt->topt.border);
...
printQueryOpt *savePsetInfo(const printQueryOpt *popt) {
  ...
  if (popt->topt.fieldSep.separator) save->topt.fieldSep.separator = pg_strdup(popt->topt.fieldSep.separator);
  if (popt->topt.recordSep.separator) save->topt.recordSep.separator = pg_strdup(popt->topt.recordSep.separator);
  if (popt->topt.tableAttr) save->topt.tableAttr = pg_strdup(popt->topt.tableAttr);
  if (popt->nullPrint) save->nullPrint = pg_strdup(popt->nullPrint);
  if (popt->title) save->title = pg_strdup(popt->title);

  /*
   * footers and translate_columns are never set in psql's print settings,
   * so we needn't write code to duplicate them.
   */
  Assert(popt->footers == nullptr);
  Assert(popt->translate_columns == nullptr);
...
static char *pset_value_string(const char *param, printQueryOpt *popt) {
  ...
  if (strcmp(param, "border") == 0)
    ...
    if (popt->topt.expanded_header_width_type == PRINT_XHEADER_FULL)
      ...
      char wbuff[32];
      snprintf(wbuff, sizeof(wbuff), "%d", popt->topt.expanded_header_exact_width);
...
#ifdef WIN32
    if (shellName == nullptr) shellName = getenv("COMSPEC");
#endif
    if (shellName == nullptr) shellName = DEFAULT_SHELL;
...
static bool do_watch(PQExpBuffer query_buf, double sleep) {
  ...
  char *title;
  const char *pagerprog = nullptr;
  FILE *pagerpipe = nullptr;
  int title_len;
  int res = 0;
#ifndef WIN32
  ...
  if (!query_buf || query_buf->len <= 0) {
    PSQL_LOG_ERROR("\\watch cannot be used with an empty query");
    return false;
  }
   ...
   */
  sigprocmask(SIG_BLOCK, &sigalrm_sigchld, nullptr);
  ...
  if (setitimer(ITIMER_REAL, &interval, nullptr) < 0) {
    PSQL_LOG_ERROR("could not set timer: %m");
    done = true;
  }
#endif
  ...
  if (pagerprog && myopt.topt.pager) {
    ...
    if (!pagerpipe) /* silently proceed without pager */
   ...
   */
  if (!pagerpipe) myopt.topt.pager = 0;
  ...
  for (;;) {
    ...
    if (pagerpipe && ferror(pagerpipe)) break;
     ...
     */
    if (sigsetjmp(sigint_interrupt_jmp, 1) != 0) break;
    ...
    sigprocmask(SIG_BLOCK, &sigint, nullptr);
    if (cancel_pressed) done = true;
  ...
  sigprocmask(SIG_UNBLOCK, &sigalrm_sigchld_sigint, nullptr);
#endif
...
static bool echo_hidden_command(const char *query) {
  if (pset.echo_hidden != PSQL_ECHO_HIDDEN_OFF) {
    ...
    if (pset.echo_hidden == PSQL_ECHO_HIDDEN_NOEXEC) return false;
  }
  return true;
}
    ...
    case EditableView:
      ...
      if (pset.sversion >= 90400) {
        printfPQExpBuffer(
            ...
            "WHEN 'check_option=cascaded' = ANY (c.reloptions) THEN 'CASCADED'::text ELSE nullptr END AS checkoption "
      ...
      case EditableView: {
        ...
        switch (relkind[0]) {
          ...
          case RELKIND_VIEW:
            ...
            break;
          default:
            PSQL_LOG_ERROR("\"%s.%s\" is not a view", nspname, relname);
            result = false;
            break;
        }
        ...
        if (reloptions != nullptr && strlen(reloptions) > 2) {
          appendPQExpBufferStr(buf, "\n WITH (");
          if (!appendReloptionsArray(buf, reloptions, "", pset.encoding, standard_strings())) {
            PSQL_LOG_ERROR("could not parse reloptions array");
            result = false;
          }
        ...
        if (buf->len > 0 && buf->data[buf->len - 1] == ';') buf->data[--(buf->len)] = '\0';

        /* WITH [LOCAL|CASCADED] CHECK OPTION */
        if (checkoption && checkoption[0] != '\0') appendPQExpBuffer(buf, "\n WITH %s CHECK OPTION", checkoption);
      } break;
    }
...
static int strip_lineno_from_objdesc(char *obj) {
  ...
  while (c > obj && isascii((unsigned char)*c) && isspace((unsigned char)*c)) c--;

  /* must have a digit as last non-space char */
  if (c == obj || !isascii((unsigned char)*c) || !isdigit((unsigned char)*c)) return -1;

  /* find start of digit string */
  while (c > obj && isascii((unsigned char)*c) && isdigit((unsigned char)*c)) c--;
  ...
  if (c == obj || !isascii((unsigned char)*c) || !(isspace((unsigned char)*c) || *c == ')')) return -1;

  /* parse digit string */
  c++;
  lineno = atoi(c);
  if (lineno < 1) {
    PSQL_LOG_ERROR("invalid line number: %s", c);
    return 0;
  }
 ...
 *
 * If header_keyword isn't nullptr, then line 1 should be the first line beginning
 * with header_keyword; lines before that are unnumbered.
 *
 * Caution: this scribbles on *lines.
 */
static void print_with_linenumbers(FILE *output, char *lines, const char *header_keyword) {
  bool in_header = (header_keyword != nullptr);
  ...
  while (*lines != '\0') {
    ...
    if (in_header && strncmp(lines, header_keyword, header_sz) == 0) in_header = false;
