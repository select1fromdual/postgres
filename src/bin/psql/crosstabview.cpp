...
static bool printCrosstab(const PGresult *result, int num_columns, pivot_field *piv_columns, int field_for_columns,
                          int num_rows, pivot_field *piv_rows, int field_for_rows, int field_for_data);
...
bool PrintResultInCrosstab(const PGresult *res) {
  ...
  if (pset.ctv_args[0] == NULL)
    ...
    if (field_for_rows < 0) goto error_return;
  }
  ...
  if (pset.ctv_args[1] == NULL)
    ...
    if (field_for_columns < 0) goto error_return;
  }
  ...
  if (pset.ctv_args[2] == NULL) {
    ...
    if (field_for_data < 0) goto error_return;
  }
  ...
  if (pset.ctv_args[3] == NULL)
    ...
    if (sort_field_for_columns < 0) goto error_return;
  }
   ...
   * header
   */
  if (sort_field_for_columns >= 0) rankSort(num_columns, array_columns);

  /*
   * Fourth part: print the crosstab'ed result.
   */
  retval = printCrosstab(res, num_columns, array_columns, field_for_columns, num_rows, array_rows, field_for_rows,
                         field_for_data);
...
static bool printCrosstab(const PGresult *result, int num_columns, pivot_field *piv_columns, int field_for_columns,
  ...
  printTableAddHeader(&cont, PQfname(result, field_for_rows), false,
  ...
  for (i = 0; i < num_columns; i++) horiz_map[piv_columns[i].rank] = i;
  ...
  for (rn = 0; rn < PQntuples(result); rn++) {
    ...
    if (col_number >= 0 && row_number >= 0) {
      ...
      cont.cells[idx] = !PQgetisnull(result, rn, field_for_data) ? PQgetvalue(result, rn, field_for_data)
  ...
  for (i = 0; i < cont.cellsadded; i++) {
    if (cont.cells[i] == NULL) cont.cells[i] = "";
  }
...
static int avlBalance(avl_node *n) { return n->children[0]->height - n->children[1]->height; }
...
static void avlAdjustBalance(avl_tree *tree, avl_node **node) {
  ...
  if (b != 0) {
    ...
    if (avlBalance(current->children[dir]) == -b) avlRotate(&current->children[dir], !dir);
    current = avlRotate(node, dir);
  }
  if (current != tree->end) avlUpdateHeight(current);
}
...
static void rankSort(int num_columns, pivot_field *piv_columns) {
  ...
  pg_qsort(hmap, num_columns, sizeof(int) * 2, rankCompare);
...
static int rankCompare(const void *a, const void *b) { return *((const int *)a) - *((const int *)b); }
