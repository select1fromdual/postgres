...
static void print_lo_result(const char *fmt, ...) {
  ...
  if (!pset.quiet) {
    if (pset.popt.topt.format == PRINT_HTML) fputs("<p>", pset.queryFout);
  ...
  *own_transaction = false;
  ...
  switch (tstatus) {
    case PQTRANS_IDLE:
      ...
      if (!(res = PSQLexec("BEGIN"))) return false;
      PQclear(res);
...
bool do_lo_export(const char *loid_arg, const char *filename_arg) {
  ...
  if (!start_lo_xact("\\lo_export", &own_transaction)) return false;
  ...
  if (!finish_lo_xact("\\lo_export", own_transaction)) return false;
...
bool do_lo_import(const char *filename_arg, const char *comment_arg) {
  ...
  if (!start_lo_xact("\\lo_import", &own_transaction)) return false;
  ...
  if (!finish_lo_xact("\\lo_import", own_transaction)) return false;
...
bool do_lo_unlink(const char *loid_arg) {
  ...
  if (!start_lo_xact("\\lo_unlink", &own_transaction)) return false;
  ...
  if (!finish_lo_xact("\\lo_unlink", own_transaction)) return false;
