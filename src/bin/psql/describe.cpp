...
static bool describeOneTSConfig(const char *oid, const char *nspname, const char *cfgname, const char *pnspname,
                                const char *prsname);
static void printACLColumn(PQExpBuffer buf, const char *colname);
static bool listOneExtensionContents(const char *extname, const char *oid);
static bool validateSQLNamePattern(PQExpBuffer buf, const char *pattern, bool have_where, bool force_escape,
                                   const char *schemavar, const char *namevar, const char *altnamevar,
                                   const char *visibilityrule, bool *added_clause, int maxparts);
...
bool describeAggregates(const char *pattern, bool verbose, bool showSystem) {
  ...
  if (!validateSQLNamePattern(&buf, pattern, true, false, "n.nspname", "p.proname", NULL,
                              "pg_catalog.pg_function_is_visible(p.oid)", NULL, 3)) {
    termPQExpBuffer(&buf);
    return false;
  }
...
bool describeAccessMethods(const char *pattern, bool verbose) {
  ...
  if (!validateSQLNamePattern(&buf, pattern, false, false, NULL, "amname", NULL, NULL, NULL, 1)) {
    termPQExpBuffer(&buf);
    return false;
  }
...
bool describeTablespaces(const char *pattern, bool verbose) {
  ...
  if (verbose) {
    ...
    appendPQExpBuffer(&buf,
                      ...
                      gettext_noop("Options"), gettext_noop("Size"), gettext_noop("Description"));
  }

  appendPQExpBufferStr(&buf, "\nFROM pg_catalog.pg_tablespace\n");

  if (!validateSQLNamePattern(&buf, pattern, false, false, NULL, "spcname", NULL, NULL, NULL, 1)) {
    termPQExpBuffer(&buf);
    return false;
  }
 ...
 */
bool describeFunctions(const char *functypes, const char *func_pattern, char **arg_patterns, int num_arg_patterns,
  ...
  PGresult *res;
  printQueryOpt myopt = pset.popt;
  static const bool translate_columns[] = {false, false, false, false, true,  true, true,
                                           false, true,  false, false, false, false};

  /* No "Parallel" column before 9.6 */
  static const bool translate_columns_pre_96[] = {false, false, false, false, true,  true,
                                                  false, true,  false, false, false, false};
  ...
  if (!showAggregate && !showNormal && !showProcedure && !showTrigger && !showWindow) {
    ...
    if (pset.sversion >= 110000) showProcedure = true;
  }
  ...
  if (pset.sversion >= 110000)
    appendPQExpBuffer(&buf,
                      ...
                      gettext_noop("agg"), gettext_noop("window"), gettext_noop("proc"), gettext_noop("func"),
                      gettext_noop("Type"));
  else
    appendPQExpBuffer(&buf,
                      "  pg_catalog.pg_get_function_result(p.oid) as \"%s\",\n"
                      ...
                      gettext_noop("Result data type"), gettext_noop("Argument data types"),
                      /* translator: "agg" is short for "aggregate" */
                      gettext_noop("agg"), gettext_noop("window"), gettext_noop("trigger"), gettext_noop("func"),
                      gettext_noop("Type"));
  ...
  if (verbose) {
    appendPQExpBuffer(&buf,
                      ...
                      " END as \"%s\"",
                      gettext_noop("immutable"), gettext_noop("stable"), gettext_noop("volatile"),
    ...
    if (pset.sversion >= 90600)
      appendPQExpBuffer(&buf,
                        ...
                        " END as \"%s\"",
                        gettext_noop("restricted"), gettext_noop("safe"), gettext_noop("unsafe"),
    ...
    if (pset.sversion >= 140000)
      ...
      appendPQExpBuffer(&buf, ",\n p.prosrc as \"%s\"", gettext_noop("Source code"));
  ...
  if (showNormal && showAggregate && showProcedure && showTrigger && showWindow)
    ...
    if (showTrigger) {
      if (needs_or) appendPQExpBufferStr(&buf, "       OR ");
      appendPQExpBufferStr(&buf, "p.prorettype = 'pg_catalog.trigger'::pg_catalog.regtype\n");
      needs_or = true;
    }
    if (showProcedure) {
  ...
  if (!validateSQLNamePattern(&buf, func_pattern, have_where, false, "n.nspname", "p.proname", NULL,
                              "pg_catalog.pg_function_is_visible(p.oid)", NULL, 3))
    goto error_return;
  ...
  for (int i = 0; i < num_arg_patterns; i++) {
    if (strcmp(arg_patterns[i], "-") != 0) {
      ...
      if (!validateSQLNamePattern(&buf, map_typename_pattern(arg_patterns[i]), true, false, nspname, typname, ft, tiv,
                                  NULL, 3))
        goto error_return;
    } else {
...
bool describeTypes(const char *pattern, bool verbose, bool showSystem) {
  ...
  if (verbose) {
    appendPQExpBuffer(&buf,
                      ...
                      gettext_noop("Internal name"), gettext_noop("Size"), gettext_noop("Elements"),
                      gettext_noop("Owner"));
  ...
  if (!validateSQLNamePattern(&buf, map_typename_pattern(pattern), true, false, "n.nspname", "t.typname",
...
static const char *map_typename_pattern(const char *pattern) {
  static const char *const typename_map[] = {
       ...
       * format_type().
       */
      "decimal", "numeric", "float", "double precision", "int", "integer",

      /*
       * We also have to map the array names for cases where the canonical
       * name is different from what pg_type says.
       */
      "bool[]", "boolean[]", "decimal[]", "numeric[]", "float[]", "double precision[]", "float4[]", "real[]",
      "float8[]", "double precision[]", "int[]", "integer[]", "int2[]", "smallint[]", "int4[]", "integer[]", "int8[]",
      "bigint[]", "time[]", "time without time zone[]", "timetz[]", "time with time zone[]", "timestamp[]",
      "timestamp without time zone[]", "timestamptz[]", "timestamp with time zone[]", "varbit[]", "bit varying[]",
      "varchar[]", "character varying[]", NULL};

  if (pattern == NULL) return NULL;
  for (int i = 0; typename_map[i] != NULL; i += 2) {
    if (pg_strcasecmp(pattern, typename_map[i]) == 0) return typename_map[i + 1];
  }
  return pattern;
}
 ...
 * Describe operators
 */
bool describeOperators(const char *oper_pattern, char **arg_patterns, int num_arg_patterns, bool verbose,
                       bool showSystem) {
  PQExpBufferData buf;
  PGresult *res;
  printQueryOpt myopt = pset.popt;
  ...
  printfPQExpBuffer(&buf,
                    ...
                    gettext_noop("Schema"), gettext_noop("Name"), gettext_noop("Left arg type"),
                    gettext_noop("Right arg type"), gettext_noop("Result type"));

  if (verbose) appendPQExpBuffer(&buf, "  o.oprcode AS \"%s\",\n", gettext_noop("Function"));
  ...
  if (!validateSQLNamePattern(&buf, oper_pattern, !showSystem && !oper_pattern, true, "n.nspname", "o.oprname", NULL,
                              "pg_catalog.pg_operator_is_visible(o.oid)", NULL, 3))
    goto error_return;

  if (num_arg_patterns == 1) appendPQExpBufferStr(&buf, "  AND o.oprleft = 0\n");
...
bool listAllDbs(const char *pattern, bool verbose) {
  ...
  printfPQExpBuffer(&buf,
                    ...
                    "       d.datcollate as \"%s\",\n"
                    "       d.datctype as \"%s\",\n",
                    gettext_noop("Name"), gettext_noop("Owner"), gettext_noop("Encoding"), gettext_noop("Collate"),
                    gettext_noop("Ctype"));
  ...
  if (verbose)
    appendPQExpBuffer(&buf,
                      ...
                      gettext_noop("Size"), gettext_noop("Tablespace"), gettext_noop("Description"));
  appendPQExpBufferStr(&buf, "\nFROM pg_catalog.pg_database d\n");
  if (verbose) appendPQExpBufferStr(&buf, "  JOIN pg_catalog.pg_tablespace t on d.dattablespace = t.oid\n");

  if (pattern) {
    if (!validateSQLNamePattern(&buf, pattern, false, false, NULL, "d.datname", NULL, NULL, NULL, 1)) {
      termPQExpBuffer(&buf);
      return false;
    }
  }
...
bool permissionsList(const char *pattern) {
   ...
   * pg_catalog.*
   */
  if (!validateSQLNamePattern(&buf, pattern, true, false, "n.nspname", "c.relname", NULL,
                              "n.nspname !~ '^pg_' AND pg_catalog.pg_table_is_visible(c.oid)", NULL, 3))
    goto error_return;
...
bool listDefaultACLs(const char *pattern) {
  ...
  printfPQExpBuffer(&buf,
                    ...
                    "  n.nspname AS \"%s\",\n"
                    "  CASE d.defaclobjtype WHEN '%c' THEN '%s' WHEN '%c' THEN '%s' WHEN '%c' THEN '%s' WHEN '%c' THEN "
                    "'%s' WHEN '%c' THEN '%s' END AS \"%s\",\n"
                    "  ",
                    gettext_noop("Owner"), gettext_noop("Schema"), DEFACLOBJ_RELATION, gettext_noop("table"),
                    DEFACLOBJ_SEQUENCE, gettext_noop("sequence"), DEFACLOBJ_FUNCTION, gettext_noop("function"),
                    DEFACLOBJ_TYPE, gettext_noop("type"), DEFACLOBJ_NAMESPACE, gettext_noop("schema"),
                    gettext_noop("Type"));
  ...
  if (!validateSQLNamePattern(&buf, pattern, false, false, NULL, "n.nspname",
                              "pg_catalog.pg_get_userbyid(d.defaclrole)", NULL, NULL, 3))
    goto error_return;
...
bool objectDescription(const char *pattern, bool showSystem) {
  ...
  if (!validateSQLNamePattern(&buf, pattern, !showSystem && !pattern, false, "n.nspname", "pgc.conname", NULL,
  ...
  if (!validateSQLNamePattern(&buf, pattern, true, false, "n.nspname", "o.opcname", NULL,
  ...
  if (!validateSQLNamePattern(&buf, pattern, true, false, "n.nspname", "opf.opfname", NULL,
  ...
  if (!validateSQLNamePattern(&buf, pattern, true, false, "n.nspname", "r.rulename", NULL,
                              "pg_catalog.pg_table_is_visible(c.oid)", NULL, 3))
    goto error_return;
  ...
  if (!validateSQLNamePattern(&buf, pattern, !showSystem && !pattern, false, "n.nspname", "t.tgname", NULL,
                              "pg_catalog.pg_table_is_visible(c.oid)", NULL, 3))
    goto error_return;
...
bool describeTableDetails(const char *pattern, bool verbose, bool showSystem) {
  ...
  if (!validateSQLNamePattern(&buf, pattern, !showSystem && !pattern, false, "n.nspname", "c.relname", NULL,
                              "pg_catalog.pg_table_is_visible(c.oid)", NULL, 3)) {
    termPQExpBuffer(&buf);
    return false;
  }
...
static bool describeOneTableDetails(const char *schemaname, const char *relationname, const char *oid, bool verbose) {
  ...
  int attname_col = -1, /* column indexes in "res" */
      atttype_col = -1, attrdef_col = -1, attnotnull_col = -1, attcoll_col = -1, attidentity_col = -1,
      attgenerated_col = -1, isindexkey_col = -1, indexdef_col = -1, fdwopts_col = -1, attstorage_col = -1,
      attcompression_col = -1, attstattarget_col = -1, attdescr_col = -1;
  int numrows;
  struct {
    int16 checks;
    char relkind;
  ...
  if (tableinfo.relkind == RELKIND_SEQUENCE) {
    ...
    if (pset.sversion >= 100000) {
      printfPQExpBuffer(&buf,
                        ...
                        "       seqcache AS \"%s\"\n",
                        gettext_noop("Type"), gettext_noop("Start"), gettext_noop("Minimum"), gettext_noop("Maximum"),
                        gettext_noop("Increment"), gettext_noop("yes"), gettext_noop("no"), gettext_noop("Cycles?"),
                        gettext_noop("Cache"));
      appendPQExpBuffer(&buf,
    ...
    if (tableinfo.relpersistence == 'u')
      printfPQExpBuffer(&title, _("Unlogged sequence \"%s.%s\""), schemaname, relationname);
    else
  ...
  if (tableinfo.relkind == RELKIND_RELATION || tableinfo.relkind == RELKIND_VIEW ||
      tableinfo.relkind == RELKIND_MATVIEW || tableinfo.relkind == RELKIND_FOREIGN_TABLE ||
      tableinfo.relkind == RELKIND_COMPOSITE_TYPE || tableinfo.relkind == RELKIND_PARTITIONED_TABLE)
  ...
  if (tableinfo.relkind == RELKIND_INDEX || tableinfo.relkind == RELKIND_PARTITIONED_INDEX) {
    if (pset.sversion >= 110000) {
      appendPQExpBuffer(&buf,
                        ",\n  CASE WHEN a.attnum <= (SELECT i.indnkeyatts FROM pg_catalog.pg_index i WHERE "
                        "i.indexrelid = '%s') THEN '%s' ELSE '%s' END AS is_key",
  ...
  if (verbose) {
    ...
    if (pset.sversion >= 140000 && !pset.hide_compression &&
        (tableinfo.relkind == RELKIND_RELATION || tableinfo.relkind == RELKIND_PARTITIONED_TABLE ||
    ...
    if (tableinfo.relkind == RELKIND_RELATION || tableinfo.relkind == RELKIND_INDEX ||
        tableinfo.relkind == RELKIND_PARTITIONED_INDEX || tableinfo.relkind == RELKIND_MATVIEW ||
        tableinfo.relkind == RELKIND_FOREIGN_TABLE || tableinfo.relkind == RELKIND_PARTITIONED_TABLE) {
      appendPQExpBufferStr(&buf,
  ...
  if (isindexkey_col >= 0) headers[cols++] = gettext_noop("Key?");
  if (indexdef_col >= 0) headers[cols++] = gettext_noop("Definition");
  if (fdwopts_col >= 0) headers[cols++] = gettext_noop("FDW options");
  if (attstorage_col >= 0) headers[cols++] = gettext_noop("Storage");
  if (attcompression_col >= 0) headers[cols++] = gettext_noop("Compression");
  if (attstattarget_col >= 0) headers[cols++] = gettext_noop("Stats target");
  if (attdescr_col >= 0) headers[cols++] = gettext_noop("Description");
  ...
  for (i = 0; i < cols; i++) printTableAddHeader(&cont, headers[i], true, 'l');
  ...
  for (i = 0; i < numrows; i++) {
    ...
    if (isindexkey_col >= 0) printTableAddCell(&cont, PQgetvalue(res, i, isindexkey_col), true, false);
    if (indexdef_col >= 0) printTableAddCell(&cont, PQgetvalue(res, i, indexdef_col), false, false);

    /* FDW options for foreign table columns */
    if (fdwopts_col >= 0) printTableAddCell(&cont, PQgetvalue(res, i, fdwopts_col), false, false);
    ...
    if (attstattarget_col >= 0) printTableAddCell(&cont, PQgetvalue(res, i, attstattarget_col), false, false);

    /* Column comments, if the relkind supports this feature */
    if (attdescr_col >= 0) printTableAddCell(&cont, PQgetvalue(res, i, attdescr_col), false, false);
  }
  ...
  if (tableinfo.ispartition) {
    ...
    appendPQExpBufferStr(&buf, pset.sversion >= 140000 ? "inhdetachpending" : "false as inhdetachpending");
    ...
    if (PQntuples(result) > 0) {
      ...
      printfPQExpBuffer(&tmpbuf, _("Partition of: %s %s%s"), parent_name, partdef,
      ...
      if (verbose) {
        ...
        if (!PQgetisnull(result, 0, 3)) partconstraintdef = PQgetvalue(result, 0, 3);
  ...
  if (tableinfo.relkind == RELKIND_TOASTVALUE) {
    ...
    if (PQntuples(result) == 1) {
      ...
      printfPQExpBuffer(&tmpbuf, _("Owning table: \"%s.%s\""), schemaname, relname);
      printTableAddFooter(&cont, tmpbuf.data);
    }
    PQclear(result);
  }
  ...
  if (tableinfo.relkind == RELKIND_INDEX || tableinfo.relkind == RELKIND_PARTITIONED_INDEX) {
    ...
    if (!result)
      ...
      if (strcmp(indisprimary, "t") == 0)
        ...
        if (strcmp(indnullsnotdistinct, "t") == 0) appendPQExpBufferStr(&tmpbuf, _(" nulls not distinct"));
      ...
      if (strlen(indpred)) appendPQExpBuffer(&tmpbuf, _(", predicate (%s)"), indpred);

      if (strcmp(indisclustered, "t") == 0) appendPQExpBufferStr(&tmpbuf, _(", clustered"));

      if (strcmp(indisvalid, "t") != 0) appendPQExpBufferStr(&tmpbuf, _(", invalid"));

      if (strcmp(deferrable, "t") == 0) appendPQExpBufferStr(&tmpbuf, _(", deferrable"));

      if (strcmp(deferred, "t") == 0) appendPQExpBufferStr(&tmpbuf, _(", initially deferred"));

      if (strcmp(indisreplident, "t") == 0) appendPQExpBufferStr(&tmpbuf, _(", replica identity"));
  ...
  /* If you add relkinds here, see also "Finish printing..." stanza below */
  else if (tableinfo.relkind == RELKIND_RELATION || tableinfo.relkind == RELKIND_MATVIEW ||
           tableinfo.relkind == RELKIND_FOREIGN_TABLE || tableinfo.relkind == RELKIND_PARTITIONED_TABLE ||
           tableinfo.relkind == RELKIND_PARTITIONED_INDEX || tableinfo.relkind == RELKIND_TOASTVALUE) {
      ...
      if (tuples > 0) {
        ...
        for (i = 0; i < tuples; i++) {
          ...
          if (strcmp(PQgetvalue(result, i, 7), "x") == 0) {
            ...
            if (usingpos) indexdef = usingpos + 7;
            appendPQExpBuffer(&buf, " %s", indexdef);

            /* Need these for deferrable PK/UNIQUE indexes */
            if (strcmp(PQgetvalue(result, i, 8), "t") == 0) appendPQExpBufferStr(&buf, " DEFERRABLE");

            if (strcmp(PQgetvalue(result, i, 9), "t") == 0) appendPQExpBufferStr(&buf, " INITIALLY DEFERRED");
          }

          /* Add these for all cases */
          if (strcmp(PQgetvalue(result, i, 3), "t") == 0) appendPQExpBufferStr(&buf, " CLUSTER");

          if (strcmp(PQgetvalue(result, i, 4), "t") != 0) appendPQExpBufferStr(&buf, " INVALID");

          if (strcmp(PQgetvalue(result, i, 10), "t") == 0) appendPQExpBufferStr(&buf, " REPLICA IDENTITY");
    ...
    if (tableinfo.hastriggers || tableinfo.relkind == RELKIND_PARTITIONED_TABLE) {
      if (pset.sversion >= 120000 && (tableinfo.ispartition || tableinfo.relkind == RELKIND_PARTITIONED_TABLE)) {
        ...
        if (pset.sversion >= 120000) appendPQExpBufferStr(&buf, "     AND conparentid = 0\n");
      ...
      if (tuples > 0) {
        int i_sametable = PQfnumber(result, "sametable"), i_conname = PQfnumber(result, "conname"),
            i_condef = PQfnumber(result, "condef"), i_ontable = PQfnumber(result, "ontable");
        ...
        for (i = 0; i < tuples; i++) {
          ...
          if (strcmp(PQgetvalue(result, i, i_sametable), "f") == 0)
            printfPQExpBuffer(&buf, "    TABLE \"%s\" CONSTRAINT \"%s\" %s", PQgetvalue(result, i, i_ontable),
                              PQgetvalue(result, i, i_conname), PQgetvalue(result, i, i_condef));
          else
    ...
    if (tableinfo.hastriggers || tableinfo.relkind == RELKIND_PARTITIONED_TABLE) {
      ...
      if (tuples > 0) {
        int i_conname = PQfnumber(result, "conname"), i_ontable = PQfnumber(result, "ontable"),
    ...
    if (pset.sversion >= 90500) {
      ...
      appendPQExpBuffer(&buf,
                        "  CASE WHEN pol.polroles = '{0}' THEN NULL ELSE pg_catalog.array_to_string(array(select "
                        "rolname from pg_catalog.pg_roles where oid = any (pol.polroles) order by 1),',') END,\n"
      ...
      if (!tableinfo.rowsecurity && tuples > 0) printTableAddFooter(&cont, _("Policies (row security disabled):"));

      /* Might be an empty set - that's ok */
      for (i = 0; i < tuples; i++) {
        printfPQExpBuffer(&buf, "    POLICY \"%s\"", PQgetvalue(result, i, 0));

        if (*(PQgetvalue(result, i, 1)) == 'f') appendPQExpBufferStr(&buf, " AS RESTRICTIVE");

        if (!PQgetisnull(result, i, 5)) appendPQExpBuffer(&buf, " FOR %s", PQgetvalue(result, i, 5));

        if (!PQgetisnull(result, i, 2)) {
          appendPQExpBuffer(&buf, "\n      TO %s", PQgetvalue(result, i, 2));
        }

        if (!PQgetisnull(result, i, 3)) appendPQExpBuffer(&buf, "\n      USING (%s)", PQgetvalue(result, i, 3));

        if (!PQgetisnull(result, i, 4)) appendPQExpBuffer(&buf, "\n      WITH CHECK (%s)", PQgetvalue(result, i, 4));
      ...
      if (tuples > 0) {
        ...
        for (i = 0; i < tuples; i++) {
          ...
          if (strcmp(PQgetvalue(result, i, 8), "-1") != 0)
            appendPQExpBuffer(&buf, "; STATISTICS %s", PQgetvalue(result, i, 8));
      ...
      if (tuples > 0) {
        ...
        for (category = 0; category < 4; category++) {
          ...
          for (i = 0; i < tuples; i++) {
            ...
            switch (category) {
              case 0:
                if (*PQgetvalue(result, i, 2) == 'O') list_rule = true;
                break;
              case 1:
                if (*PQgetvalue(result, i, 2) == 'D') list_rule = true;
                break;
              case 2:
                if (*PQgetvalue(result, i, 2) == 'A') list_rule = true;
                break;
              case 3:
                if (*PQgetvalue(result, i, 2) == 'R') list_rule = true;
                break;
            }
            if (!list_rule) continue;
    ...
    if (pset.sversion >= 100000) {
      ...
      if (tuples > 0) printTableAddFooter(&cont, _("Publications:"));
      ...
      for (i = 0; i < tuples; i++) {
        ...
        /* column list (if any) */
        if (!PQgetisnull(result, i, 2)) appendPQExpBuffer(&buf, " (%s)", PQgetvalue(result, i, 2));

        /* row filter (if any) */
        if (!PQgetisnull(result, i, 1)) appendPQExpBuffer(&buf, " WHERE %s", PQgetvalue(result, i, 1));
  ...
  if ((tableinfo.relkind == RELKIND_VIEW || tableinfo.relkind == RELKIND_MATVIEW) && verbose) {
    ...
    if (PQntuples(result) > 0) view_def = pg_strdup(PQgetvalue(result, 0, 0));
    ...
    if (tuples > 0) {
      ...
      for (category = 0; category <= 4; category++) {
        ...
        for (i = 0; i < tuples; i++) {
          ...
          switch (category) {
            case 0:
              if (*tgenabled == 'O' || *tgenabled == 't') list_trigger = true;
              break;
            case 1:
              if ((*tgenabled == 'D' || *tgenabled == 'f') && *tgisinternal == 'f') list_trigger = true;
              break;
            case 2:
              if ((*tgenabled == 'D' || *tgenabled == 'f') && *tgisinternal == 't') list_trigger = true;
              break;
            case 3:
              if (*tgenabled == 'A') list_trigger = true;
              break;
            case 4:
              if (*tgenabled == 'R') list_trigger = true;
              break;
          }
          if (list_trigger == false) continue;
          ...
          if (usingpos) tgdef = usingpos + 9;

          printfPQExpBuffer(&buf, "    %s", tgdef);

          /* Visually distinguish inherited triggers */
          if (!PQgetisnull(result, i, 4)) appendPQExpBuffer(&buf, ", ON TABLE %s", PQgetvalue(result, i, 4));
   ...
   */
  if (tableinfo.relkind == RELKIND_RELATION || tableinfo.relkind == RELKIND_MATVIEW ||
      tableinfo.relkind == RELKIND_FOREIGN_TABLE || tableinfo.relkind == RELKIND_PARTITIONED_TABLE ||
      tableinfo.relkind == RELKIND_PARTITIONED_INDEX || tableinfo.relkind == RELKIND_TOASTVALUE) {
    bool is_partitioned;
    PGresult *result;
    ...
    printfPQExpBuffer(&buf,
                      ...
                      "  AND c.relkind != " CppAsString2(RELKIND_PARTITIONED_TABLE) " AND c.relkind != " CppAsString2(
    ...
    if (!result)
      ...
      for (i = 0; i < tuples; i++) {
        ...
        if (i < tuples - 1) appendPQExpBufferChar(&buf, ',');
    ...
    if (is_partitioned && tuples == 0) {
      ...
      for (i = 0; i < tuples; i++) {
        ...
        if (!PQgetisnull(result, i, 3)) appendPQExpBuffer(&buf, " %s", PQgetvalue(result, i, 3));
        if (child_relkind == RELKIND_PARTITIONED_TABLE || child_relkind == RELKIND_PARTITIONED_INDEX)
          appendPQExpBufferStr(&buf, ", PARTITIONED");
        if (strcmp(PQgetvalue(result, i, 2), "t") == 0) appendPQExpBufferStr(&buf, " (DETACH PENDING)");
        if (i < tuples - 1) appendPQExpBufferChar(&buf, ',');
...
error_return:
  ...
  /* clean up */
  if (printTableInitialized) printTableCleanup(&cont);
  termPQExpBuffer(&buf);
...
static void add_tablespace_footer(printTableContent *const cont, char relkind, Oid tablespace, const bool newline) {
  ...
  if (relkind == RELKIND_RELATION || relkind == RELKIND_MATVIEW || relkind == RELKIND_INDEX ||
      relkind == RELKIND_PARTITIONED_TABLE || relkind == RELKIND_PARTITIONED_INDEX || relkind == RELKIND_TOASTVALUE) {
    /*
...
bool describeRoles(const char *pattern, bool verbose, bool showSystem) {
  ...
  if (!validateSQLNamePattern(&buf, pattern, false, false, NULL, "r.rolname", NULL, NULL, NULL, 1)) {
    termPQExpBuffer(&buf);
    return false;
  }
  ...
  if (verbose) printTableAddHeader(&cont, gettext_noop("Description"), true, align);

  for (i = 0; i < nrows; i++) {
    printTableAddCell(&cont, PQgetvalue(res, i, 0), false, false);

    resetPQExpBuffer(&buf);
    if (strcmp(PQgetvalue(res, i, 1), "t") == 0) add_role_attribute(&buf, _("Superuser"));

    if (strcmp(PQgetvalue(res, i, 2), "t") != 0) add_role_attribute(&buf, _("No inheritance"));

    if (strcmp(PQgetvalue(res, i, 3), "t") == 0) add_role_attribute(&buf, _("Create role"));

    if (strcmp(PQgetvalue(res, i, 4), "t") == 0) add_role_attribute(&buf, _("Create DB"));

    if (strcmp(PQgetvalue(res, i, 5), "t") != 0) add_role_attribute(&buf, _("Cannot login"));

    if (strcmp(PQgetvalue(res, i, (verbose ? 10 : 9)), "t") == 0) add_role_attribute(&buf, _("Replication"));

    if (pset.sversion >= 90500)
      if (strcmp(PQgetvalue(res, i, (verbose ? 11 : 10)), "t") == 0) add_role_attribute(&buf, _("Bypass RLS"));
    ...
    if (verbose) printTableAddCell(&cont, PQgetvalue(res, i, 9), false, false);
  }
  termPQExpBuffer(&buf);
...
static void add_role_attribute(PQExpBuffer buf, const char *const str) {
  if (buf->len > 0) appendPQExpBufferStr(buf, ", ");
...
bool listDbRoleSettings(const char *pattern, const char *pattern2) {
  ...
  if (!validateSQLNamePattern(&buf, pattern, false, false, NULL, "r.rolname", NULL, NULL, &havewhere, 1))
    goto error_return;
  if (!validateSQLNamePattern(&buf, pattern2, havewhere, false, NULL, "d.datname", NULL, NULL, NULL, 1))
    goto error_return;
  appendPQExpBufferStr(&buf, "ORDER BY 1, 2;");
...
bool listTables(const char *tabtypes, const char *pattern, bool verbose, bool showSystem) {
  ...
  if (verbose) {
    ...
    appendPQExpBuffer(
        ...
        gettext_noop("permanent"), gettext_noop("temporary"), gettext_noop("unlogged"), gettext_noop("Persistence"));
  ...
  if (showTables) {
    ...
    if (showSystem || pattern) appendPQExpBufferStr(&buf, CppAsString2(RELKIND_TOASTVALUE) ",");
  }
  if (showViews) appendPQExpBufferStr(&buf, CppAsString2(RELKIND_VIEW) ",");
  if (showMatViews) appendPQExpBufferStr(&buf, CppAsString2(RELKIND_MATVIEW) ",");
  if (showIndexes)
    appendPQExpBufferStr(&buf, CppAsString2(RELKIND_INDEX) "," CppAsString2(RELKIND_PARTITIONED_INDEX) ",");
  if (showSeq) appendPQExpBufferStr(&buf, CppAsString2(RELKIND_SEQUENCE) ",");
  if (showSystem || pattern) appendPQExpBufferStr(&buf, "'s',"); /* was RELKIND_SPECIAL */
  if (showForeign) appendPQExpBufferStr(&buf, CppAsString2(RELKIND_FOREIGN_TABLE) ",");
...
bool listPartitionedTables(const char *reltypes, const char *pattern, bool verbose) {
  ...
  appendPQExpBufferStr(&buf, "\nWHERE c.relkind IN (");
  if (showTables) appendPQExpBufferStr(&buf, CppAsString2(RELKIND_PARTITIONED_TABLE) ",");
  if (showIndexes) appendPQExpBufferStr(&buf, CppAsString2(RELKIND_PARTITIONED_INDEX) ",");
  appendPQExpBufferStr(&buf, "''"); /* dummy */
  appendPQExpBufferStr(&buf, ")\n");
  ...
  appendPQExpBuffer(&buf, "ORDER BY \"Schema\", %s%s\"Name\";", mixed_output ? "\"Type\" DESC, " : "",
...
bool listLanguages(const char *pattern, bool verbose, bool showSystem) {
  ...
  printfPQExpBuffer(&buf,
                    ...
                    "       l.lanpltrusted AS \"%s\"",
                    gettext_noop("Name"), gettext_noop("Owner"), gettext_noop("Trusted"));
  ...
  if (pattern) {
    if (!validateSQLNamePattern(&buf, pattern, false, false, NULL, "l.lanname", NULL, NULL, NULL, 2)) {
      termPQExpBuffer(&buf);
      return false;
    }
  }

  if (!showSystem && !pattern) appendPQExpBufferStr(&buf, "WHERE l.lanplcallfoid != 0\n");
...
bool listDomains(const char *pattern, bool verbose, bool showSystem) {
  ...
  if (!validateSQLNamePattern(&buf, pattern, true, false, "n.nspname", "t.typname", NULL,
                              "pg_catalog.pg_type_is_visible(t.oid)", NULL, 3)) {
    termPQExpBuffer(&buf);
    return false;
  }
...
bool listConversions(const char *pattern, bool verbose, bool showSystem) {
  ...
  printfPQExpBuffer(&buf,
                    ...
                    "       ELSE '%s' END AS \"%s\"",
                    gettext_noop("Schema"), gettext_noop("Name"), gettext_noop("Source"), gettext_noop("Destination"),
                    gettext_noop("yes"), gettext_noop("no"), gettext_noop("Default?"));

  if (verbose) appendPQExpBuffer(&buf, ",\n       d.description AS \"%s\"", gettext_noop("Description"));
  ...
  if (!validateSQLNamePattern(&buf, pattern, true, false, "n.nspname", "c.conname", NULL,
                              "pg_catalog.pg_conversion_is_visible(c.oid)", NULL, 3)) {
    termPQExpBuffer(&buf);
    return false;
  }
...
bool describeConfigurationParameters(const char *pattern, bool verbose, bool showSystem) {
  ...
  printfPQExpBuffer(&buf,
                    ...
                    gettext_noop("Parameter"), gettext_noop("Value"));

  if (verbose) {
    appendPQExpBuffer(&buf, ", s.vartype AS \"%s\", s.context AS \"%s\", ", gettext_noop("Type"),
                      gettext_noop("Context"));
    if (pset.sversion >= 150000)
      printACLColumn(&buf, "p.paracl");
    else
      appendPQExpBuffer(&buf, "NULL AS \"%s\"", gettext_noop("Access privileges"));
  }
  ...
  if (pattern)
    processSQLNamePattern(pset.db, &buf, pattern, false, false, NULL, "pg_catalog.lower(s.name)", NULL, NULL, NULL,
                          NULL);
  else
    appendPQExpBufferStr(&buf,
...
bool listEventTriggers(const char *pattern, bool verbose) {
  ...
  printfPQExpBuffer(&buf,
                    ...
                    gettext_noop("Name"), gettext_noop("Event"), gettext_noop("Owner"), gettext_noop("enabled"),
                    gettext_noop("replica"), gettext_noop("always"), gettext_noop("disabled"), gettext_noop("Enabled"),
  ...
  if (!validateSQLNamePattern(&buf, pattern, false, false, NULL, "evtname", NULL, NULL, NULL, 1)) {
    termPQExpBuffer(&buf);
    return false;
  }
...
bool listExtendedStats(const char *pattern) {
  ...
  appendPQExpBuffer(&buf,
                    ...
                    "END AS \"%s\"",
                    gettext_noop("Ndistinct"), gettext_noop("Dependencies"));
  ...
  if (!validateSQLNamePattern(&buf, pattern, false, false, "es.stxnamespace::pg_catalog.regnamespace::pg_catalog.text",
                              "es.stxname", NULL, "pg_catalog.pg_statistics_obj_is_visible(es.oid)", NULL, 3)) {
    termPQExpBuffer(&buf);
    return false;
  }
...
bool listCasts(const char *pattern, bool verbose) {
  ...
  appendPQExpBuffer(&buf,
                    ...
                    "            ELSE '%s'\n"
                    "       END AS \"%s\"",
                    COERCION_CODE_EXPLICIT, gettext_noop("no"), COERCION_CODE_ASSIGNMENT, gettext_noop("in assignment"),
   ...
   */
  if (!validateSQLNamePattern(&buf, pattern, true, false, "ns.nspname", "ts.typname",
                              "pg_catalog.format_type(ts.oid, NULL)", "pg_catalog.pg_type_is_visible(ts.oid)", NULL, 3))
    goto error_return;

  appendPQExpBufferStr(&buf, ") OR (true");

  if (!validateSQLNamePattern(&buf, pattern, true, false, "nt.nspname", "tt.typname",
                              "pg_catalog.format_type(tt.oid, NULL)", "pg_catalog.pg_type_is_visible(tt.oid)", NULL, 3))
    goto error_return;
...
bool listCollations(const char *pattern, bool verbose, bool showSystem) {
  ...
  if (pset.sversion >= 150000)
    appendPQExpBuffer(&buf, ",\n       c.colliculocale AS \"%s\"", gettext_noop("ICU Locale"));
  else
    appendPQExpBuffer(&buf, ",\n       c.collcollate AS \"%s\"", gettext_noop("ICU Locale"));
  ...
  if (pset.sversion >= 120000)
    appendPQExpBuffer(&buf, ",\n       CASE WHEN c.collisdeterministic THEN '%s' ELSE '%s' END AS \"%s\"",
                      gettext_noop("yes"), gettext_noop("no"), gettext_noop("Deterministic?"));
  else
  ...
  if (!validateSQLNamePattern(&buf, pattern, true, false, "n.nspname", "c.collname", NULL,
                              "pg_catalog.pg_collation_is_visible(c.oid)", NULL, 3)) {
    termPQExpBuffer(&buf);
    return false;
  }
...
bool listSchemas(const char *pattern, bool verbose, bool showSystem) {
  ...
  if (!validateSQLNamePattern(&buf, pattern, !showSystem && !pattern, false, NULL, "n.nspname", NULL, NULL, NULL, 2))
    goto error_return;
  ...
  if (footers) {
    ...
    for (footer = footers; *footer; footer++) pg_free(*footer);
...
bool listTSParsers(const char *pattern, bool verbose) {
  ...
  if (verbose) return listTSParsersVerbose(pattern);
  ...
  if (!validateSQLNamePattern(&buf, pattern, false, false, "n.nspname", "p.prsname", NULL,
                              "pg_catalog.pg_ts_parser_is_visible(p.oid)", NULL, 3)) {
    termPQExpBuffer(&buf);
    return false;
  }
...
static bool listTSParsersVerbose(const char *pattern) {
  ...
  for (i = 0; i < PQntuples(res); i++) {
    ...
    oid = PQgetvalue(res, i, 0);
    if (!PQgetisnull(res, i, 1)) nspname = PQgetvalue(res, i, 1);
...
static bool describeOneTSParser(const char *oid, const char *nspname, const char *prsname) {
  ...
  printfPQExpBuffer(&buf,
                    ...
                    " FROM pg_catalog.pg_ts_parser p\n"
                    " WHERE p.oid = '%s';",
                    gettext_noop("Start parse"), gettext_noop("Method"), gettext_noop("Function"),
                    gettext_noop("Description"), oid, gettext_noop("Get next token"), oid, gettext_noop("End parse"),
                    oid, gettext_noop("Get headline"), oid, gettext_noop("Get token types"), oid);
...
bool listTSDictionaries(const char *pattern, bool verbose) {
  ...
  if (!validateSQLNamePattern(&buf, pattern, false, false, "n.nspname", "d.dictname", NULL,
                              "pg_catalog.pg_ts_dict_is_visible(d.oid)", NULL, 3)) {
    termPQExpBuffer(&buf);
    return false;
  }
...
bool listTSTemplates(const char *pattern, bool verbose) {
  ...
  if (verbose)
    printfPQExpBuffer(&buf,
                      ...
                      gettext_noop("Schema"), gettext_noop("Name"), gettext_noop("Init"), gettext_noop("Lexize"),
                      gettext_noop("Description"));
  else
    printfPQExpBuffer(&buf,
                      "SELECT\n"
  ...
  if (!validateSQLNamePattern(&buf, pattern, false, false, "n.nspname", "t.tmplname", NULL,
                              "pg_catalog.pg_ts_template_is_visible(t.oid)", NULL, 3)) {
    termPQExpBuffer(&buf);
    return false;
  }
...
bool listTSConfigs(const char *pattern, bool verbose) {
  ...
  if (verbose) return listTSConfigsVerbose(pattern);
  ...
  if (!validateSQLNamePattern(&buf, pattern, false, false, "n.nspname", "c.cfgname", NULL,
                              "pg_catalog.pg_ts_config_is_visible(c.oid)", NULL, 3)) {
    termPQExpBuffer(&buf);
    return false;
  }
...
static bool listTSConfigsVerbose(const char *pattern) {
  ...
  if (!validateSQLNamePattern(&buf, pattern, true, false, "n.nspname", "c.cfgname", NULL,
                              "pg_catalog.pg_ts_config_is_visible(c.oid)", NULL, 3)) {
    termPQExpBuffer(&buf);
    return false;
  }
  ...
  for (i = 0; i < PQntuples(res); i++) {
    ...
    if (!PQgetisnull(res, i, 2)) nspname = PQgetvalue(res, i, 2);
    prsname = PQgetvalue(res, i, 3);
    if (!PQgetisnull(res, i, 4)) pnspname = PQgetvalue(res, i, 4);
...
bool listForeignDataWrappers(const char *pattern, bool verbose) {
  ...
  printfPQExpBuffer(&buf,
                    ...
                    gettext_noop("Name"), gettext_noop("Owner"), gettext_noop("Handler"), gettext_noop("Validator"));
  ...
  if (!validateSQLNamePattern(&buf, pattern, false, false, NULL, "fdwname", NULL, NULL, NULL, 1)) {
    termPQExpBuffer(&buf);
    return false;
  }
...
bool listForeignServers(const char *pattern, bool verbose) {
  ...
  if (verbose) {
    ...
    appendPQExpBuffer(&buf,
                      ...
                      "  END AS \"%s\",\n"
                      "  d.description AS \"%s\"",
                      gettext_noop("Type"), gettext_noop("Version"), gettext_noop("FDW options"),
                      gettext_noop("Description"));
  }
  ...
  if (!validateSQLNamePattern(&buf, pattern, false, false, NULL, "s.srvname", NULL, NULL, NULL, 1)) {
    termPQExpBuffer(&buf);
    return false;
  }
...
bool listUserMappings(const char *pattern, bool verbose) {
  ...
  if (!validateSQLNamePattern(&buf, pattern, false, false, NULL, "um.srvname", "um.usename", NULL, NULL, 1)) {
    termPQExpBuffer(&buf);
    return false;
  }
...
bool listForeignTables(const char *pattern, bool verbose) {
  ...
  if (!validateSQLNamePattern(&buf, pattern, false, false, "n.nspname", "c.relname", NULL,
                              "pg_catalog.pg_table_is_visible(c.oid)", NULL, 3)) {
    termPQExpBuffer(&buf);
    return false;
  }
...
bool listExtensions(const char *pattern) {
  ...
  if (!validateSQLNamePattern(&buf, pattern, false, false, NULL, "e.extname", NULL, NULL, NULL, 1)) {
    termPQExpBuffer(&buf);
    return false;
  }
...
static bool validateSQLNamePattern(PQExpBuffer buf, const char *pattern, bool have_where, bool force_escape,
  ...
  added = processSQLNamePattern(pset.db, buf, pattern, have_where, force_escape, schemavar, namevar, altnamevar,
...
bool listPublications(const char *pattern) {
  ...
  printfPQExpBuffer(&buf,
                    ...
                    "  pubupdate AS \"%s\",\n"
                    "  pubdelete AS \"%s\"",
                    gettext_noop("Name"), gettext_noop("Owner"), gettext_noop("All tables"), gettext_noop("Inserts"),
                    gettext_noop("Updates"), gettext_noop("Deletes"));
  if (pset.sversion >= 110000) appendPQExpBuffer(&buf, ",\n  pubtruncate AS \"%s\"", gettext_noop("Truncates"));
  if (pset.sversion >= 130000) appendPQExpBuffer(&buf, ",\n  pubviaroot AS \"%s\"", gettext_noop("Via root"));

  appendPQExpBufferStr(&buf, "\nFROM pg_catalog.pg_publication\n");

  if (!validateSQLNamePattern(&buf, pattern, false, false, NULL, "pubname", NULL, NULL, NULL, 1)) {
    termPQExpBuffer(&buf);
    return false;
  }
 ...
 */
static bool addFooterToPublicationDesc(PQExpBuffer buf, const char *footermsg, bool as_schema,
  ...
  if (count > 0) printTableAddFooter(cont, footermsg);

  for (i = 0; i < count; i++) {
    if (as_schema)
      printfPQExpBuffer(buf, "    \"%s\"", PQgetvalue(res, i, 0));
    else {
      printfPQExpBuffer(buf, "    \"%s.%s\"", PQgetvalue(res, i, 0), PQgetvalue(res, i, 1));

      if (!PQgetisnull(res, i, 3)) appendPQExpBuffer(buf, " (%s)", PQgetvalue(res, i, 3));

      if (!PQgetisnull(res, i, 2)) appendPQExpBuffer(buf, " WHERE %s", PQgetvalue(res, i, 2));
    }
...
bool describePublications(const char *pattern) {
  ...
  if (has_pubtruncate) appendPQExpBufferStr(&buf, ", pubtruncate");
  if (has_pubviaroot) appendPQExpBufferStr(&buf, ", pubviaroot");
  appendPQExpBufferStr(&buf, "\nFROM pg_catalog.pg_publication\n");
  ...
  for (i = 0; i < PQntuples(res); i++) {
    ...
    if (has_pubtruncate) printTableAddHeader(&cont, gettext_noop("Truncates"), true, align);
    if (has_pubviaroot) printTableAddHeader(&cont, gettext_noop("Via root"), true, align);
    ...
    if (has_pubtruncate) printTableAddCell(&cont, PQgetvalue(res, i, 7), false, false);
    if (has_pubviaroot) printTableAddCell(&cont, PQgetvalue(res, i, 8), false, false);
    ...
    if (!puballtables) {
      ...
      appendPQExpBuffer(&buf,
                        ...
                        "ORDER BY 1,2",
                        pubid);
      if (!addFooterToPublicationDesc(&buf, _("Tables:"), false, &cont)) goto error_return;
      ...
      if (pset.sversion >= 150000) {
        ...
        printfPQExpBuffer(&buf,
                          ...
                          "ORDER BY 1",
                          pubid);
        if (!addFooterToPublicationDesc(&buf, _("Tables from schemas:"), true, &cont)) goto error_return;
      }
    }
...
bool describeSubscriptions(const char *pattern, bool verbose) {
  ...
  if (verbose) {
    ...
    if (pset.sversion >= 140000)
      appendPQExpBuffer(&buf,
                        ", subbinary AS \"%s\"\n"
                        ", substream AS \"%s\"\n",
                        gettext_noop("Binary"), gettext_noop("Streaming"));
    ...
    if (pset.sversion >= 150000)
      appendPQExpBuffer(&buf,
                        ...
                        gettext_noop("Two-phase commit"), gettext_noop("Disable on error"));

    if (pset.sversion >= 160000) appendPQExpBuffer(&buf, ", suborigin AS \"%s\"\n", gettext_noop("Origin"));
    ...
    if (pset.sversion >= 150000) appendPQExpBuffer(&buf, ", subskiplsn AS \"%s\"\n", gettext_noop("Skip LSN"));
  }
  ...
  if (!validateSQLNamePattern(&buf, pattern, true, false, NULL, "subname", NULL, NULL, NULL, 1)) {
    termPQExpBuffer(&buf);
    return false;
  }
...
bool listOperatorClasses(const char *access_method_pattern, const char *type_pattern, bool verbose) {
  ...
  printfPQExpBuffer(&buf,
                    ...
                    "    ELSE '%s'\n"
                    "  END) AS \"%s\"",
                    gettext_noop("AM"), gettext_noop("Input type"), gettext_noop("Storage type"),
                    gettext_noop("Operator class"), gettext_noop("yes"), gettext_noop("no"), gettext_noop("Default?"));
  if (verbose)
    appendPQExpBuffer(&buf,
                      ",\n  CASE\n"
                      ...
                      gettext_noop("Operator family"), gettext_noop("Owner"));
  appendPQExpBufferStr(&buf,
  ...
  if (access_method_pattern)
    if (!validateSQLNamePattern(&buf, access_method_pattern, false, false, NULL, "am.amname", NULL, NULL, &have_where,
                                1))
      goto error_return;
  if (type_pattern) {
    /* Match type name pattern against either internal or external name */
    if (!validateSQLNamePattern(&buf, type_pattern, have_where, false, "tn.nspname", "t.typname",
                                "pg_catalog.format_type(t.oid, NULL)", "pg_catalog.pg_type_is_visible(t.oid)", NULL, 3))
      goto error_return;
  }
...
bool listOperatorFamilies(const char *access_method_pattern, const char *type_pattern, bool verbose) {
  ...
  printfPQExpBuffer(&buf,
                    ...
                    gettext_noop("AM"), gettext_noop("Operator family"), gettext_noop("Applicable types"));
  if (verbose)
  ...
  if (type_pattern) {
    ...
    /* Match type name pattern against either internal or external name */
    if (!validateSQLNamePattern(&buf, type_pattern, true, false, "tn.nspname", "t.typname",
                                "pg_catalog.format_type(t.oid, NULL)", "pg_catalog.pg_type_is_visible(t.oid)", NULL, 3))
      goto error_return;
    appendPQExpBufferStr(&buf, "  )\n");
  }
...
bool listOpFamilyOperators(const char *access_method_pattern, const char *family_pattern, bool verbose) {
  ...
  printfPQExpBuffer(&buf,
                    ...
                    "    WHEN 's' THEN '%s'\n"
                    "  END AS \"%s\"\n",
                    gettext_noop("AM"), gettext_noop("Operator family"), gettext_noop("Operator"),
                    gettext_noop("Strategy"), gettext_noop("ordering"), gettext_noop("search"),
                    gettext_noop("Purpose"));

  if (verbose) appendPQExpBuffer(&buf, ", ofs.opfname AS \"%s\"\n", gettext_noop("Sort opfamily"));
  appendPQExpBufferStr(&buf,
  ...
  if (family_pattern) {
    if (!validateSQLNamePattern(&buf, family_pattern, have_where, false, "nsf.nspname", "of.opfname", NULL, NULL, NULL,
                                3))
      goto error_return;
  }
...
bool listOpFamilyFunctions(const char *access_method_pattern, const char *family_pattern, bool verbose) {
  ...
  printfPQExpBuffer(&buf,
                    ...
                    "  ap.amprocnum AS \"%s\"\n",
                    gettext_noop("AM"), gettext_noop("Operator family"), gettext_noop("Registered left type"),
                    gettext_noop("Registered right type"), gettext_noop("Number"));
  ...
  if (family_pattern) {
    if (!validateSQLNamePattern(&buf, family_pattern, have_where, false, "ns.nspname", "of.opfname", NULL, NULL, NULL,
                                3))
      goto error_return;
  }
