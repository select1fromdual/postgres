...
int main(int argc, char *argv[]) {
   ...
   * work in this case.
   */
  if (options.actions.head == NULL && pset.notty) simple_action_list_append(&options.actions, ACT_FILE, NULL);

  /* Bail out if -1 was specified but will be ignored. */
  if (options.single_txn && options.actions.head == NULL) pg_fatal("-1 can only be used in non-interactive mode");
...
#define PARAMS_ARRAY_SIZE 8
  ...
  if (options.logfilename) {
    ...
    if (!pset.logfile) pg_fatal("could not open log file \"%s\": %m", options.logfilename);
  }
  ...
  if (options.actions.head != NULL) {
    ...
    for (cell = options.actions.head; cell; cell = cell->next) {
      if (cell->action == ACT_SINGLE_QUERY) {
        ...
        successResult =
            HandleSlashCmds(scan_state, cond_stack, NULL, NULL) != PSQL_CMD_ERROR ? EXIT_SUCCESS : EXIT_FAILURE;
...
static void parse_psql_options(int argc, char *argv[], struct adhoc_opts *options) {
  static struct option long_options[] = {{"echo-all", no_argument, NULL, 'a'},
  ...
  while ((c = getopt_long(argc, argv, "aAbc:d:eEf:F:h:HlL:no:p:P:qR:sStT:U:v:VwWxXz?01", long_options, &optindex)) !=
    ...
    switch (c) {
      ...
      case 'P': {
        ...
        if (!result) pg_fatal("could not set printing parameter \"%s\"", value);
...
static void process_psqlrc(char *argv0) {
  ...
  if (find_my_exec(argv0, my_exec_path) < 0) pg_fatal("could not find own program executable");
...
static char *fetch_count_substitute_hook(char *newval) {
  if (newval == NULL) newval = pg_strdup("0");
  return newval;
}
...
static char *histsize_substitute_hook(char *newval) {
  if (newval == NULL) newval = pg_strdup("500");
  return newval;
}
...
static char *echo_substitute_hook(char *newval) {
  if (newval == NULL) newval = pg_strdup("none");
  return newval;
}
...
static char *comp_keyword_case_substitute_hook(char *newval) {
  if (newval == NULL) newval = pg_strdup("preserve-upper");
  return newval;
}
...
static char *verbosity_substitute_hook(char *newval) {
  if (newval == NULL) newval = pg_strdup("default");
  return newval;
}
...
static char *show_context_substitute_hook(char *newval) {
  if (newval == NULL) newval = pg_strdup("errors");
  return newval;
}
