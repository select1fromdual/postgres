 ...
 * results).
 */
#define COMPLETE_WITH_QUERY(query) COMPLETE_WITH_QUERY_LIST(query, NULL)
...
#define COMPLETE_WITH_QUERY_VERBATIM(query) COMPLETE_WITH_QUERY_VERBATIM_LIST(query, NULL)
...
#define COMPLETE_WITH_VERSIONED_QUERY(query) COMPLETE_WITH_VERSIONED_QUERY_LIST(query, NULL)
...
#define COMPLETE_WITH_SCHEMA_QUERY(query) COMPLETE_WITH_SCHEMA_QUERY_LIST(query, NULL)
...
#define COMPLETE_WITH_VERSIONED_SCHEMA_QUERY(query) COMPLETE_WITH_VERSIONED_SCHEMA_QUERY_LIST(query, NULL)
...
#define COMPLETE_WITH_ATTR(relation) COMPLETE_WITH_ATTR_LIST(relation, NULL)
...
#define COMPLETE_WITH_ENUM_VALUE(type)                                      \
  do {                                                                      \
    ...
    if (text[0] == '\'' || start == 0 || rl_line_buffer[start - 1] != '\'') \
...
static const char *const Keywords_for_list_of_datatypes[] = {"bigint",
                                                             "boolean",
                                                             "character",
                                                             "double precision",
                                                             "integer",
...
#define Keywords_for_list_of_grant_roles "PUBLIC", "CURRENT_ROLE", "CURRENT_USER", "SESSION_USER"
...
static const VersionedQuery Query_for_list_of_publications[] = {{100000,
                                                                 " SELECT pubname "
...
static const char *const Keywords_for_user_thing[] = {"MAPPING FOR", NULL};
...
static const char *const table_storage_parameters[] = {"autovacuum_analyze_scale_factor",
...
static char *_complete_from_query(const char *simple_query, const SchemaQuery *schema_query,
                                  const char *const *keywords, bool verbatim, const char *text, int state);
...
static void append_variable_names(char ***varnames, int *nvars, int *maxvars, const char *varname, const char *prefix,
                                  const char *suffix);
...
static void parse_identifier(const char *ident, char **schemaname, char **objectname, bool *schemaquoted,
                             bool *objectquoted);
static char *requote_identifier(const char *schemaname, const char *objectname, bool quote_schema, bool quote_object);
...
void initialize_readline(void) {
...
#ifdef HAVE_RL_FILENAME_QUOTE_CHARACTERS
  {
    ...
    for (int i = 0; i < 255; i++) fqc[i] = (unsigned char)(i + 1);
    fqc[255] = '\0';
...
static bool word_matches(const char *pattern, const char *word, bool case_sensitive) {
  size_t wordlen;
...
#define cimatch(s1, s2, n) (case_sensitive ? strncmp(s1, s2, n) == 0 : pg_strncasecmp(s1, s2, n) == 0)
  ...
  if (*pattern == '!') return !word_matches(pattern + 1, word, case_sensitive);
  ...
  for (;;) {
    ...
    if (star) {
      ...
      if (wordlen >= (beforelen + afterlen) && cimatch(word, pattern, beforelen) &&
          cimatch(word + wordlen - afterlen, star + 1, afterlen))
        return true;
    } else {
      /* No, plain match? */
      if (wordlen == (c - pattern) && cimatch(word, pattern, wordlen)) return true;
    }
    /* Out of alternatives? */
    if (*c == '\0') break;
 ...
 */
static bool TailMatchesImpl(bool case_sensitive, int previous_words_count, char **previous_words, int narg, ...) {
  va_list args;

  if (previous_words_count < narg) return false;
 ...
 * in previous_words match the variadic arguments?
 */
static bool MatchesImpl(bool case_sensitive, int previous_words_count, char **previous_words, int narg, ...) {
  va_list args;
 ...
 */
static bool HeadMatchesImpl(bool case_sensitive, int previous_words_count, char **previous_words, int narg, ...) {
  va_list args;
...
#define TailMatches(...) \
  TailMatchesImpl(false, previous_words_count, previous_words, VA_ARGS_NARGS(__VA_ARGS__), __VA_ARGS__)

  /* Match the last N words before point, case-sensitively. */
#define TailMatchesCS(...) \
  TailMatchesImpl(true, previous_words_count, previous_words, VA_ARGS_NARGS(__VA_ARGS__), __VA_ARGS__)
...
#define HeadMatches(...) \
  HeadMatchesImpl(false, previous_words_count, previous_words, VA_ARGS_NARGS(__VA_ARGS__), __VA_ARGS__)

  /* Match the first N words on the line, case-sensitively. */
#define HeadMatchesCS(...) \
  HeadMatchesImpl(true, previous_words_count, previous_words, VA_ARGS_NARGS(__VA_ARGS__), __VA_ARGS__)
  ...
  static const char *const backslash_commands[] = {"\\a",
                                                   "\\connect",
                                                   "\\conninfo",
                                                   "\\C",
                                                   "\\cd",
  ...
  if (text[0] == '\\') COMPLETE_WITH_LIST_CS(backslash_commands);
  ...
  /* complete with something you can create or replace */
    ...
    COMPLETE_WITH("FUNCTION", "PROCEDURE", "LANGUAGE", "RULE", "VIEW", "AGGREGATE", "TRANSFORM", "TRIGGER");
  ...
  /* ALTER TABLE */
    ...
    COMPLETE_WITH_SCHEMA_QUERY_PLUS(Query_for_list_of_tables, "ALL IN TABLESPACE");
  ...
  /* ALTER FUNCTION,PROCEDURE,ROUTINE <name> (...) */
    ...
    if (ends_with(prev_wd, ')'))
      COMPLETE_WITH("OWNER TO", "RENAME TO", "SET SCHEMA", "DEPENDS ON EXTENSION", "NO DEPENDS ON EXTENSION");
    else
      COMPLETE_WITH_FUNCTION_ARG(prev2_wd);
  }
  ...
  /* ALTER PUBLICATION <name> ADD */
    ...
    COMPLETE_WITH("ALL TABLES IN SCHEMA", "TABLE");
           ...
           (HeadMatches("ALTER", "PUBLICATION", MatchAny, "ADD|SET", "TABLE") && ends_with(prev_wd, ',')))
  ...
  else if (HeadMatches("ALTER", "PUBLICATION", MatchAny, "ADD|SET", "TABLE") && !TailMatches("WHERE", "(*)"))
    COMPLETE_WITH(",", "WHERE (");
  ...
  /* ALTER PUBLICATION <name> SET */
    ...
    COMPLETE_WITH_QUERY_PLUS(Query_for_list_of_schemas " AND nspname NOT LIKE E'pg\\\\_%%'", "CURRENT_SCHEMA");
  /* ALTER PUBLICATION <name> SET ( */
  ...
  /* ALTER SUBSCRIPTION <name> */
    ...
    COMPLETE_WITH("CONNECTION", "ENABLE", "DISABLE", "OWNER TO", "RENAME TO", "REFRESH PUBLICATION", "SET", "SKIP (",
  ...
  /* ALTER SUBSCRIPTION <name> SET ( */
    ...
    COMPLETE_WITH("binary", "disable_on_error", "origin", "slot_name", "streaming", "synchronous_commit");
  ...
  else if (HeadMatches("ALTER", "SUBSCRIPTION", MatchAny) && TailMatches("ADD|DROP|SET", "PUBLICATION", MatchAny))
    COMPLETE_WITH("WITH (");
  ...
  /* ALTER DATABASE <name> */
    ...
    COMPLETE_WITH("RESET", "SET", "OWNER TO", "REFRESH COLLATION VERSION", "RENAME TO", "IS_TEMPLATE",
  ...
  /* ALTER FOREIGN DATA WRAPPER <name> */
    ...
    COMPLETE_WITH("HANDLER", "VALIDATOR", "NO", "OPTIONS", "OWNER TO", "RENAME TO");
  ...
  /* ALTER FOREIGN TABLE <name> */
    ...
    COMPLETE_WITH("ADD", "ALTER", "DISABLE TRIGGER", "DROP", "ENABLE", "INHERIT", "NO INHERIT", "OPTIONS", "OWNER TO",
                  "RENAME", "SET", "VALIDATE CONSTRAINT");

  /* ALTER INDEX */
  else if (Matches("ALTER", "INDEX"))
    COMPLETE_WITH_SCHEMA_QUERY_PLUS(Query_for_list_of_indexes, "ALL IN TABLESPACE");
  /* ALTER INDEX <name> */
  else if (Matches("ALTER", "INDEX", MatchAny))
    COMPLETE_WITH("ALTER COLUMN", "OWNER TO", "RENAME TO", "SET", "RESET", "ATTACH PARTITION", "DEPENDS ON EXTENSION",
  ...
  /* ALTER INDEX <foo> SET|RESET ( */
    ...
    COMPLETE_WITH("fillfactor", "deduplicate_items",      /* BTREE */
  ...
  /* ALTER MATERIALIZED VIEW */
    ...
    COMPLETE_WITH_SCHEMA_QUERY_PLUS(Query_for_list_of_matviews, "ALL IN TABLESPACE");

  /* ALTER USER,ROLE <name> */
  else if (Matches("ALTER", "USER|ROLE", MatchAny) && !TailMatches("USER", "MAPPING"))
    COMPLETE_WITH("BYPASSRLS", "CONNECTION LIMIT", "CREATEDB", "CREATEROLE", "ENCRYPTED PASSWORD", "INHERIT", "LOGIN",
                  "NOBYPASSRLS", "NOCREATEDB", "NOCREATEROLE", "NOINHERIT", "NOLOGIN", "NOREPLICATION", "NOSUPERUSER",
                  "PASSWORD", "RENAME TO", "REPLICATION", "RESET", "SET", "SUPERUSER", "VALID UNTIL", "WITH");
  ...
  /* ALTER USER,ROLE <name> WITH */
    ...
    COMPLETE_WITH("BYPASSRLS", "CONNECTION LIMIT", "CREATEDB", "CREATEROLE", "ENCRYPTED PASSWORD", "INHERIT", "LOGIN",
                  "NOBYPASSRLS", "NOCREATEDB", "NOCREATEROLE", "NOINHERIT", "NOLOGIN", "NOREPLICATION", "NOSUPERUSER",
                  "PASSWORD", "RENAME TO", "REPLICATION", "RESET", "SET", "SUPERUSER", "VALID UNTIL");
  ...
  else if (Matches("ALTER", "DEFAULT", "PRIVILEGES", "FOR", "ROLE|USER", MatchAny, "IN", "SCHEMA", MatchAny) ||
           Matches("ALTER", "DEFAULT", "PRIVILEGES", "IN", "SCHEMA", MatchAny, "FOR", "ROLE|USER", MatchAny))
  ...
  /* ALTER SEQUENCE <name> */
    ...
    COMPLETE_WITH("AS", "INCREMENT", "MINVALUE", "MAXVALUE", "RESTART", "NO", "CACHE", "CYCLE", "SET", "OWNED BY",
                  "OWNER TO", "RENAME TO");
  ...
  /* ALTER SYSTEM SET, RESET, RESET ALL */
    ...
    COMPLETE_WITH_QUERY_VERBATIM_PLUS(Query_for_list_of_alter_system_set_vars, "ALL");
  else if (Matches("ALTER", "SYSTEM", "SET", MatchAny))
    COMPLETE_WITH("TO");
  /* ALTER VIEW <name> */
  else if (Matches("ALTER", "VIEW", MatchAny))
    COMPLETE_WITH("ALTER COLUMN", "OWNER TO", "RENAME", "SET SCHEMA");
  ...
  /* ALTER MATERIALIZED VIEW <name> */
    ...
    COMPLETE_WITH("ALTER COLUMN", "CLUSTER ON", "DEPENDS ON EXTENSION", "NO DEPENDS ON EXTENSION", "OWNER TO", "RENAME",
                  "RESET (", "SET");
  ...
  /* ALTER POLICY <name> ON <table> TO <role> */
    ...
    COMPLETE_WITH_QUERY_PLUS(Query_for_list_of_roles, Keywords_for_list_of_grant_roles);
   ...
   */
    ...
    COMPLETE_WITH("ADD", "ALTER", "CLUSTER ON", "DISABLE", "DROP", "ENABLE", "INHERIT", "NO", "RENAME", "RESET",
                  "OWNER TO", "SET", "VALIDATE CONSTRAINT", "REPLICA IDENTITY", "ATTACH PARTITION", "DETACH PARTITION",
                  "FORCE ROW LEVEL SECURITY", "OF", "NOT OF");
  /* ALTER TABLE xxx ADD */
  else if (Matches("ALTER", "TABLE", MatchAny, "ADD")) {
    /* make sure to keep this list and the !Matches() below in sync */
    COMPLETE_WITH("COLUMN", "CONSTRAINT", "CHECK", "UNIQUE", "PRIMARY KEY", "EXCLUDE", "FOREIGN KEY");
  }
  ...
  else if (Matches("ALTER", "TABLE", MatchAny, "ADD", "CONSTRAINT", MatchAny, "PRIMARY", "KEY", "USING", "INDEX")) {
  ...
  /* If we have ALTER TABLE <sth> SET, provide list of attributes and '(' */
    ...
    COMPLETE_WITH("(", "ACCESS METHOD", "LOGGED", "SCHEMA", "TABLESPACE", "UNLOGGED", "WITH", "WITHOUT");
  ...
  /* ALTER TABLESPACE <foo> SET|RESET ( */
    ...
    COMPLETE_WITH("seq_page_cost", "random_page_cost", "effective_io_concurrency", "maintenance_io_concurrency");
  ...
  /* complete ALTER TYPE <foo> SET ( with settable properties */
    ...
    COMPLETE_WITH("ANALYZE", "RECEIVE", "SEND", "STORAGE", "SUBSCRIPT", "TYPMOD_IN", "TYPMOD_OUT");
   ...
   */
    ...
    COMPLETE_WITH_SCHEMA_QUERY_PLUS(Query_for_list_of_analyzables, "VERBOSE");
  ...
  /* CLOSE */
    ...
    COMPLETE_WITH_QUERY_PLUS(Query_for_list_of_cursors, "ALL");
  /* CLUSTER */
  else if (Matches("CLUSTER"))
    COMPLETE_WITH_SCHEMA_QUERY_PLUS(Query_for_list_of_clusterables, "VERBOSE");
  else if (Matches("CLUSTER", "VERBOSE") || Matches("CLUSTER", "(*)"))
  ...
  /* If we have CLUSTER <sth> USING, then add the index as well */
    ...
    set_completion_reference(prev2_wd);
    COMPLETE_WITH_SCHEMA_QUERY(Query_for_index_of_table);
  } else if (HeadMatches("CLUSTER", "(*") && !HeadMatches("CLUSTER", "(*)")) {
    /*
     * This fires if we're in an unfinished parenthesized option list.
     * get_previous_words treats a completed parenthesized option list as
     * one word, so the above test is correct.
     */
    if (ends_with(prev_wd, '(') || ends_with(prev_wd, ',')) COMPLETE_WITH("VERBOSE");
  }
  ...
  /* COMMENT */
    ...
    COMPLETE_WITH("ACCESS METHOD", "AGGREGATE", "CAST", "COLLATION", "COLUMN", "CONSTRAINT", "CONVERSION", "DATABASE",
                  "DOMAIN", "EXTENSION", "EVENT TRIGGER", "FOREIGN DATA WRAPPER", "FOREIGN TABLE", "FUNCTION", "INDEX",
                  "LANGUAGE", "LARGE OBJECT", "MATERIALIZED VIEW", "OPERATOR", "POLICY", "PROCEDURE",
                  "PROCEDURAL LANGUAGE", "PUBLICATION", "ROLE", "ROUTINE", "RULE", "SCHEMA", "SEQUENCE", "SERVER",
                  "STATISTICS", "SUBSCRIPTION", "TABLE", "TABLESPACE", "TEXT SEARCH", "TRANSFORM FOR", "TRIGGER",
                  "TYPE", "VIEW");
    ...
    set_completion_reference(prev2_wd);
    COMPLETE_WITH_SCHEMA_QUERY_PLUS(Query_for_list_of_tables_for_constraint, "DOMAIN");
  ...
  /* Complete COPY <sth> FROM|TO filename WITH ( */
    ...
    COMPLETE_WITH("FORMAT", "FREEZE", "DELIMITER", "NULL", "HEADER", "QUOTE", "ESCAPE", "FORCE_QUOTE", "FORCE_NOT_NULL",
                  "FORCE_NULL", "ENCODING");
  ...
  /* CREATE COLLATION */
    ...
    if (TailMatches("(|*,"))
      COMPLETE_WITH("LOCALE =", "LC_COLLATE =", "LC_CTYPE =", "PROVIDER =", "DETERMINISTIC =");
  ...
  /* CREATE DATABASE */
    ...
    COMPLETE_WITH("OWNER", "TEMPLATE", "ENCODING", "TABLESPACE", "IS_TEMPLATE", "STRATEGY", "ALLOW_CONNECTIONS",
                  "CONNECTION LIMIT", "LC_COLLATE", "LC_CTYPE", "LOCALE", "OID", "LOCALE_PROVIDER", "ICU_LOCALE");
  ...
  /* CREATE DOMAIN */
    ...
    COMPLETE_WITH("COLLATE", "DEFAULT", "CONSTRAINT", "NOT NULL", "NULL", "CHECK (");
   ...
   */
    ...
    COMPLETE_WITH_SCHEMA_QUERY_PLUS(Query_for_list_of_indexes, "ON", "CONCURRENTLY");
   ...
   */
    ...
    COMPLETE_WITH_SCHEMA_QUERY_PLUS(Query_for_list_of_indexes, "ON");
  ...
  /* Complete USING with an index method */
           ...
           TailMatches("INDEX", MatchAny, "ON", MatchAny, "USING") || TailMatches("INDEX", "ON", MatchAny, "USING"))
  ...
  else if (Matches("CREATE", "RULE", MatchAny) || Matches("CREATE", "OR", "REPLACE", "RULE", MatchAny))
    COMPLETE_WITH("AS ON");
  /* Complete "CREATE [ OR REPLACE ] RULE <sth> AS" with "ON" */
  else if (Matches("CREATE", "RULE", MatchAny, "AS") || Matches("CREATE", "OR", "REPLACE", "RULE", MatchAny, "AS"))
    COMPLETE_WITH("ON");
  ...
  /* CREATE SEQUENCE --- is allowed inside CREATE SCHEMA, so use TailMatches */
    ...
    COMPLETE_WITH("AS", "INCREMENT BY", "MINVALUE", "MAXVALUE", "NO", "CACHE", "CYCLE", "OWNED BY", "START WITH");
  ...
  /* CREATE TRANSFORM */
  else if (Matches("CREATE", "TRANSFORM") || Matches("CREATE", "OR", "REPLACE", "TRANSFORM"))
    COMPLETE_WITH("FOR");
  else if (Matches("CREATE", "TRANSFORM", "FOR") || Matches("CREATE", "OR", "REPLACE", "TRANSFORM", "FOR"))
    COMPLETE_WITH_SCHEMA_QUERY(Query_for_list_of_datatypes);
  ...
  /* Complete "CREATE SUBSCRIPTION <name> ...  WITH ( <opt>" */
    ...
    COMPLETE_WITH("binary", "connect", "copy_data", "create_slot", "disable_on_error", "enabled", "origin", "slot_name",
   ...
   * OF.
   */
  else if (TailMatches("CREATE", "TRIGGER", MatchAny) || TailMatches("CREATE", "OR", "REPLACE", "TRIGGER", MatchAny))
   ...
   */
    ...
    COMPLETE_WITH("ROW", "STATEMENT");
           ...
           (TailMatches("FOR", "EACH", "ROW|STATEMENT") || TailMatches("FOR", "ROW|STATEMENT"))) {
    if (pset.sversion >= 110000)
  ...
  /* CREATE ROLE,USER,GROUP <name> */
    ...
    COMPLETE_WITH("ADMIN", "BYPASSRLS", "CONNECTION LIMIT", "CREATEDB", "CREATEROLE", "ENCRYPTED PASSWORD", "IN",
                  "INHERIT", "LOGIN", "NOBYPASSRLS", "NOCREATEDB", "NOCREATEROLE", "NOINHERIT", "NOLOGIN",
                  "NOREPLICATION", "NOSUPERUSER", "PASSWORD", "REPLICATION", "ROLE", "SUPERUSER", "SYSID",
                  "VALID UNTIL", "WITH");
  ...
  /* CREATE TYPE */
    ...
    if (TailMatches("(|*,"))
      COMPLETE_WITH("INPUT", "OUTPUT", "RECEIVE", "SEND", "TYPMOD_IN", "TYPMOD_OUT", "ANALYZE", "SUBSCRIPT",
                    "INTERNALLENGTH", "PASSEDBYVALUE", "ALIGNMENT", "STORAGE", "LIKE", "CATEGORY", "PREFERRED",
                    "DEFAULT", "ELEMENT", "DELIMITER", "COLLATABLE");
    ...
    if (TailMatches("(|*,"))
      COMPLETE_WITH("SUBTYPE", "SUBTYPE_OPCLASS", "COLLATION", "CANONICAL", "SUBTYPE_DIFF", "MULTIRANGE_TYPE_NAME");
    else if (TailMatches("(*|*,", MatchAnyExcept("*=")))
      COMPLETE_WITH("=");
    else if (TailMatches("=", MatchAnyExcept("*)")))
      COMPLETE_WITH(",", ")");
  ...
  else if (TailMatches("CREATE", "VIEW", MatchAny) || TailMatches("CREATE", "OR", "REPLACE", "VIEW", MatchAny))
    COMPLETE_WITH("AS");
  ...
  /* Complete CREATE EVENT TRIGGER <name> ON with event_type */
    ...
    COMPLETE_WITH("ddl_command_start", "ddl_command_end", "sql_drop", "table_rewrite");
  ...
  /* DEALLOCATE */
    ...
    COMPLETE_WITH_QUERY_PLUS(Query_for_list_of_prepared_statements, "ALL");
   ...
   */
    ...
    COMPLETE_WITH("BINARY", "ASENSITIVE", "INSENSITIVE", "SCROLL", "NO SCROLL", "CURSOR");
  ...
  /* Complete DROP object with CASCADE / RESTRICT */
                   ...
                   "COLLATION|CONVERSION|DOMAIN|EXTENSION|LANGUAGE|PUBLICATION|SCHEMA|SEQUENCE|SERVER|SUBSCRIPTION|"
                   "STATISTICS|TABLE|TYPE|VIEW",
                   MatchAny) ||
           Matches("DROP", "ACCESS", "METHOD", MatchAny) ||
           (Matches("DROP", "AGGREGATE|FUNCTION|PROCEDURE|ROUTINE", MatchAny, MatchAny) && ends_with(prev_wd, ')')) ||
           Matches("DROP", "EVENT", "TRIGGER", MatchAny) || Matches("DROP", "FOREIGN", "DATA", "WRAPPER", MatchAny) ||
  ...
  /* DROP INDEX */
    ...
    COMPLETE_WITH_SCHEMA_QUERY_PLUS(Query_for_list_of_indexes, "CONCURRENTLY");
   ...
   */
    ...
    COMPLETE_WITH("SELECT", "INSERT INTO", "DELETE FROM", "UPDATE", "DECLARE", "MERGE", "EXECUTE", "ANALYZE",
                  "VERBOSE");
     ...
     */
    if (ends_with(prev_wd, '(') || ends_with(prev_wd, ','))
      COMPLETE_WITH("ANALYZE", "VERBOSE", "COSTS", "SETTINGS", "BUFFERS", "WAL", "TIMING", "SUMMARY", "FORMAT");
    else if (TailMatches("ANALYZE|VERBOSE|COSTS|SETTINGS|BUFFERS|WAL|TIMING|SUMMARY"))
      COMPLETE_WITH("ON", "OFF");
    else if (TailMatches("FORMAT"))
      COMPLETE_WITH("TEXT", "XML", "JSON", "YAML");
  } else if (Matches("EXPLAIN", "ANALYZE"))
    COMPLETE_WITH("SELECT", "INSERT INTO", "DELETE FROM", "UPDATE", "DECLARE", "MERGE", "EXECUTE", "VERBOSE");
  else if (Matches("EXPLAIN", "(*)") || Matches("EXPLAIN", "VERBOSE") || Matches("EXPLAIN", "ANALYZE", "VERBOSE"))
    COMPLETE_WITH("SELECT", "INSERT INTO", "DELETE FROM", "UPDATE", "DECLARE", "MERGE", "EXECUTE");
   ...
   */
    ...
    COMPLETE_WITH_QUERY_PLUS(Query_for_list_of_cursors, "ABSOLUTE", "BACKWARD", "FORWARD", "RELATIVE", "ALL", "NEXT",
                             "PRIOR", "FIRST", "LAST", "FROM", "IN");
   ...
   */
    ...
    COMPLETE_WITH_QUERY_PLUS(Query_for_list_of_cursors, "ALL", "FROM", "IN");
   ...
   */
  else if (Matches("FETCH|MOVE", "ABSOLUTE|BACKWARD|FORWARD|RELATIVE", MatchAnyExcept("FROM|IN")) ||
           Matches("FETCH|MOVE", "ALL|NEXT|PRIOR|FIRST|LAST"))
    COMPLETE_WITH_QUERY_PLUS(Query_for_list_of_cursors, "FROM", "IN");
  /* Complete FETCH <direction> "FROM" or "IN" with a list of cursors */
  else if (HeadMatches("FETCH|MOVE") && TailMatches("FROM|IN"))
  ...
  else if (TailMatches("GRANT|REVOKE") || TailMatches("REVOKE", "GRANT", "OPTION", "FOR")) {
    /*
     * With ALTER DEFAULT PRIVILEGES, restrict completion to grantable
     * privileges (can't grant roles)
     */
    if (HeadMatches("ALTER", "DEFAULT", "PRIVILEGES"))
      COMPLETE_WITH("SELECT", "INSERT", "UPDATE", "DELETE", "TRUNCATE", "REFERENCES", "TRIGGER", "EXECUTE", "USAGE",
                    "ALL");
    else
      COMPLETE_WITH_QUERY_PLUS(Query_for_list_of_roles, "GRANT", "SELECT", "INSERT", "UPDATE", "DELETE", "TRUNCATE",
                               "REFERENCES", "TRIGGER", "CREATE", "CONNECT", "TEMPORARY", "EXECUTE", "USAGE", "SET",
                               "ALTER SYSTEM", "ALL");
  }
  ...
  else if (TailMatches("GRANT|REVOKE", "ALTER") || TailMatches("REVOKE", "GRANT", "OPTION", "FOR", "ALTER"))
    COMPLETE_WITH("SYSTEM");

  else if (TailMatches("GRANT|REVOKE", "SET") || TailMatches("REVOKE", "GRANT", "OPTION", "FOR", "SET") ||
   ...
   * TO/FROM
   */
  else if (TailMatches("GRANT|REVOKE", MatchAny) || TailMatches("REVOKE", "GRANT", "OPTION", "FOR", MatchAny)) {
    if (TailMatches(
   ...
   */
    ...
    if (HeadMatches("ALTER", "DEFAULT", "PRIVILEGES"))
      ...
      COMPLETE_WITH_SCHEMA_QUERY_PLUS(Query_for_list_of_grantables, "ALL FUNCTIONS IN SCHEMA",
                                      "ALL PROCEDURES IN SCHEMA", "ALL ROUTINES IN SCHEMA", "ALL SEQUENCES IN SCHEMA",
                                      "ALL TABLES IN SCHEMA", "DATABASE", "DOMAIN", "FOREIGN DATA WRAPPER",
                                      "FOREIGN SERVER", "FUNCTION", "LANGUAGE", "LARGE OBJECT", "PARAMETER",
                                      "PROCEDURE", "ROUTINE", "SCHEMA", "SEQUENCE", "TABLE", "TABLESPACE", "TYPE");
  } else if (TailMatches("GRANT|REVOKE", MatchAny, "ON", "ALL") ||
             TailMatches("REVOKE", "GRANT", "OPTION", "FOR", MatchAny, "ON", "ALL"))
    COMPLETE_WITH("FUNCTIONS IN SCHEMA", "PROCEDURES IN SCHEMA", "ROUTINES IN SCHEMA", "SEQUENCES IN SCHEMA",
                  "TABLES IN SCHEMA");
   ...
   */
    ...
    COMPLETE_WITH("WITH ADMIN OPTION", "WITH GRANT OPTION", "GRANTED BY");
  else if (HeadMatches("GRANT") && TailMatches("TO", MatchAny, "WITH"))
    COMPLETE_WITH("ADMIN OPTION", "GRANT OPTION");
  ...
  else if (TailMatches("LIMIT", "TO", "(*)") || TailMatches("EXCEPT", "(*)"))
  ...
  /* Complete LOCK [TABLE] [ONLY] with a list of tables */
    ...
    COMPLETE_WITH_SCHEMA_QUERY_PLUS(Query_for_list_of_tables, "TABLE", "ONLY");
  else if (Matches("LOCK", "TABLE"))
    COMPLETE_WITH_SCHEMA_QUERY_PLUS(Query_for_list_of_tables, "ONLY");
  else if (Matches("LOCK", "TABLE", "ONLY") || Matches("LOCK", "ONLY"))
    COMPLETE_WITH_SCHEMA_QUERY(Query_for_list_of_tables);
  /* For the following, handle the case of a single table only for now */

  /* Complete LOCK [TABLE] [ONLY] <table> with IN or NOWAIT */
  else if (Matches("LOCK", MatchAnyExcept("TABLE|ONLY")) || Matches("LOCK", "TABLE", MatchAnyExcept("ONLY")) ||
           Matches("LOCK", "ONLY", MatchAny) || Matches("LOCK", "TABLE", "ONLY", MatchAny))
    COMPLETE_WITH("IN", "NOWAIT");

  /* Complete LOCK [TABLE] [ONLY] <table> IN with a lock mode */
  else if (HeadMatches("LOCK") && TailMatches("IN"))
    COMPLETE_WITH("ACCESS SHARE MODE", "ROW SHARE MODE", "ROW EXCLUSIVE MODE", "SHARE UPDATE EXCLUSIVE MODE",
  ...
  /* REFRESH MATERIALIZED VIEW */
    ...
    COMPLETE_WITH_SCHEMA_QUERY_PLUS(Query_for_list_of_matviews, "CONCURRENTLY");
  ...
  else if (Matches("REINDEX", "TABLE") || Matches("REINDEX", "(*)", "TABLE"))
    COMPLETE_WITH_SCHEMA_QUERY_PLUS(Query_for_list_of_indexables, "CONCURRENTLY");
  else if (Matches("REINDEX", "INDEX") || Matches("REINDEX", "(*)", "INDEX"))
    COMPLETE_WITH_SCHEMA_QUERY_PLUS(Query_for_list_of_indexes, "CONCURRENTLY");
  else if (Matches("REINDEX", "SCHEMA") || Matches("REINDEX", "(*)", "SCHEMA"))
    COMPLETE_WITH_QUERY_PLUS(Query_for_list_of_schemas, "CONCURRENTLY");
  else if (Matches("REINDEX", "SYSTEM|DATABASE") || Matches("REINDEX", "(*)", "SYSTEM|DATABASE"))
    COMPLETE_WITH_QUERY_PLUS(Query_for_list_of_databases, "CONCURRENTLY");
  else if (Matches("REINDEX", "TABLE", "CONCURRENTLY") || Matches("REINDEX", "(*)", "TABLE", "CONCURRENTLY"))
    COMPLETE_WITH_SCHEMA_QUERY(Query_for_list_of_indexables);
  else if (Matches("REINDEX", "INDEX", "CONCURRENTLY") || Matches("REINDEX", "(*)", "INDEX", "CONCURRENTLY"))
    COMPLETE_WITH_SCHEMA_QUERY(Query_for_list_of_indexes);
  else if (Matches("REINDEX", "SCHEMA", "CONCURRENTLY") || Matches("REINDEX", "(*)", "SCHEMA", "CONCURRENTLY"))
    COMPLETE_WITH_QUERY(Query_for_list_of_schemas);
  else if (Matches("REINDEX", "SYSTEM|DATABASE", "CONCURRENTLY") ||
           Matches("REINDEX", "(*)", "SYSTEM|DATABASE", "CONCURRENTLY"))
    COMPLETE_WITH_QUERY(Query_for_list_of_databases);
  else if (HeadMatches("REINDEX", "(*") && !HeadMatches("REINDEX", "(*)")) {
    /*
     * This fires if we're in an unfinished parenthesized option list.
     * get_previous_words treats a completed parenthesized option list as
     * one word, so the above test is correct.
  ...
  /* SECURITY LABEL */
    ...
    COMPLETE_WITH("TABLE", "COLUMN", "AGGREGATE", "DATABASE", "DOMAIN", "EVENT TRIGGER", "FOREIGN TABLE", "FUNCTION",
                  "LARGE OBJECT", "MATERIALIZED VIEW", "LANGUAGE", "PUBLICATION", "PROCEDURE", "ROLE", "ROUTINE",
                  "SCHEMA", "SEQUENCE", "SUBSCRIPTION", "TABLESPACE", "TYPE", "VIEW");
  ...
  /* Complete with a variable name */
    ...
    COMPLETE_WITH_QUERY_VERBATIM_PLUS(Query_for_list_of_set_vars, "CONSTRAINTS", "TRANSACTION", "SESSION", "ROLE",
                                      "TABLESPACE", "ALL");
  else if (Matches("SHOW"))
    COMPLETE_WITH_QUERY_VERBATIM_PLUS(Query_for_list_of_show_vars, "SESSION AUTHORIZATION", "ALL");
  ...
  /* Complete "SET TRANSACTION" */
  ...
  else if (Matches("BEGIN|START", "TRANSACTION") || Matches("BEGIN", "WORK") || Matches("BEGIN") ||
           Matches("SET", "SESSION", "CHARACTERISTICS", "AS", "TRANSACTION"))
    COMPLETE_WITH("ISOLATION LEVEL", "READ", "DEFERRABLE", "NOT DEFERRABLE");
  else if (Matches("SET|BEGIN|START", "TRANSACTION|WORK", "NOT") || Matches("BEGIN", "NOT") ||
           Matches("SET", "SESSION", "CHARACTERISTICS", "AS", "TRANSACTION", "NOT"))
    COMPLETE_WITH("DEFERRABLE");
  else if (Matches("SET|BEGIN|START", "TRANSACTION|WORK", "ISOLATION") || Matches("BEGIN", "ISOLATION") ||
    ...
    COMPLETE_WITH("READ");
  else if (Matches("SET|BEGIN|START", "TRANSACTION|WORK", "READ") || Matches("BEGIN", "READ") ||
           Matches("SET", "SESSION", "CHARACTERISTICS", "AS", "TRANSACTION", "READ"))
    COMPLETE_WITH("ONLY", "WRITE");
  /* SET CONSTRAINTS */
  else if (Matches("SET", "CONSTRAINTS"))
    COMPLETE_WITH_SCHEMA_QUERY_PLUS(Query_for_list_of_constraints_with_schema, "ALL");
  ...
  /* Complete SET SESSION AUTHORIZATION with username */
    ...
    COMPLETE_WITH_QUERY_PLUS(Query_for_list_of_roles, "DEFAULT");
   ...
   * <name>
   */
  else if (HeadMatches("ALTER", "DATABASE|FUNCTION|PROCEDURE|ROLE|ROUTINE|USER") && TailMatches("SET", MatchAny))
   ...
   */
    ...
    if (TailMatches("DateStyle", "TO|="))
      COMPLETE_WITH("ISO", "SQL", "Postgres", "German", "YMD", "DMY", "MDY", "US", "European", "NonEuropean",
                    "DEFAULT");
      ...
      if (guctype) {
        if (strcmp(guctype, "enum") == 0) {
          ...
          COMPLETE_WITH_QUERY_PLUS(Query_for_values_of_enum_GUC, "DEFAULT");
  ...
  /* TRUNCATE */
    ...
    COMPLETE_WITH_SCHEMA_QUERY_PLUS(Query_for_list_of_truncatables, "TABLE", "ONLY");
  else if (Matches("TRUNCATE", "TABLE"))
    COMPLETE_WITH_SCHEMA_QUERY_PLUS(Query_for_list_of_truncatables, "ONLY");
  else if (HeadMatches("TRUNCATE") && TailMatches("ONLY"))
    COMPLETE_WITH_SCHEMA_QUERY(Query_for_list_of_truncatables);
  else if (Matches("TRUNCATE", MatchAny) || Matches("TRUNCATE", "TABLE|ONLY", MatchAny) ||
  ...
  /* USER MAPPING */
    ...
    COMPLETE_WITH_QUERY_PLUS(Query_for_list_of_roles, "CURRENT_ROLE", "CURRENT_USER", "PUBLIC", "USER");
   ...
   */
    ...
    COMPLETE_WITH_SCHEMA_QUERY_PLUS(Query_for_list_of_vacuumables, "FULL", "FREEZE", "ANALYZE", "VERBOSE");
  else if (Matches("VACUUM", "FULL"))
    COMPLETE_WITH_SCHEMA_QUERY_PLUS(Query_for_list_of_vacuumables, "FREEZE", "ANALYZE", "VERBOSE");
  else if (Matches("VACUUM", "FREEZE") || Matches("VACUUM", "FULL", "FREEZE"))
    COMPLETE_WITH_SCHEMA_QUERY_PLUS(Query_for_list_of_vacuumables, "VERBOSE", "ANALYZE");
  else if (Matches("VACUUM", "VERBOSE") || Matches("VACUUM", "FULL|FREEZE", "VERBOSE") ||
           Matches("VACUUM", "FULL", "FREEZE", "VERBOSE"))
    COMPLETE_WITH_SCHEMA_QUERY_PLUS(Query_for_list_of_vacuumables, "ANALYZE");
     ...
     */
    if (ends_with(prev_wd, '(') || ends_with(prev_wd, ','))
      COMPLETE_WITH("FULL", "FREEZE", "ANALYZE", "VERBOSE", "DISABLE_PAGE_SKIPPING", "SKIP_LOCKED", "INDEX_CLEANUP",
  ...
  /* TODO:  \dc \dd \dl */
    ...
    if (!recognized_connection_string(text)) COMPLETE_WITH_QUERY(Query_for_list_of_databases);
  } else if (TailMatchesCS("\\connect|\\c", MatchAny)) {
    if (!recognized_connection_string(prev_wd)) COMPLETE_WITH_QUERY(Query_for_list_of_roles);
  } else if (TailMatchesCS("\\da*"))
    COMPLETE_WITH_VERSIONED_SCHEMA_QUERY(Query_for_list_of_aggregates);
  else if (TailMatchesCS("\\dAc*", MatchAny) || TailMatchesCS("\\dAf*", MatchAny))
    COMPLETE_WITH_SCHEMA_QUERY(Query_for_list_of_datatypes);
  else if (TailMatchesCS("\\dAo*", MatchAny) || TailMatchesCS("\\dAp*", MatchAny))
  ...
  /* must be at end of \d alternatives: */
    ...
    COMPLETE_WITH_CS("border", "columns", "csv_fieldsep", "expanded", "fieldsep", "fieldsep_zero", "footer", "format",
                     "linestyle", "null", "numericlocale", "pager", "pager_min_lines", "recordsep", "recordsep_zero",
                     "tableattr", "title", "tuples_only", "unicode_border_linestyle", "unicode_column_linestyle",
                     "unicode_header_linestyle", "xheader_width");
  else if (TailMatchesCS("\\pset", MatchAny)) {
    if (TailMatchesCS("format"))
      COMPLETE_WITH_CS("aligned", "asciidoc", "csv", "html", "latex", "latex-longtable", "troff-ms", "unaligned",
                       "wrapped");
   ...
   */
    ...
    for (wac = words_after_create; wac->name != NULL; wac++) {
      if (pg_strcasecmp(prev_wd, wac->name) == 0) {
        if (wac->query)
          COMPLETE_WITH_QUERY_LIST(wac->query, wac->keywords);
        else if (wac->vquery)
          COMPLETE_WITH_VERSIONED_QUERY_LIST(wac->vquery, wac->keywords);
        else if (wac->squery)
          COMPLETE_WITH_VERSIONED_SCHEMA_QUERY_LIST(wac->squery, wac->keywords);
        break;
      }
    }
  }
...
static char *complete_from_query(const char *text, int state) {
  ...
  return _complete_from_query(completion_charp, NULL, completion_charpp, completion_verbatim, text, state);
}
...
static char *complete_from_versioned_query(const char *text, int state) {
  ...
  while (pset.sversion < vquery->min_server_version) vquery++;
  /* Fail completion if server is too old */
  if (vquery->query == NULL) return NULL;

  return _complete_from_query(vquery->query, NULL, completion_charpp, completion_verbatim, text, state);
}

static char *complete_from_schema_query(const char *text, int state) {
  /* query is assumed to work for any server version */
  return _complete_from_query(NULL, completion_squery, completion_charpp, completion_verbatim, text, state);
}
...
static char *complete_from_versioned_schema_query(const char *text, int state) {
  ...
  /* Find appropriate array element */
  while (pset.sversion < squery->min_server_version) squery++;
  /* Fail completion if server is too old */
  if (squery->catname == NULL) return NULL;

  return _complete_from_query(NULL, squery, completion_charpp, completion_verbatim, text, state);
}
...
static char *_complete_from_query(const char *simple_query, const SchemaQuery *schema_query,
  ...
  if (state == 0) {
    ...
    if (verbatim) {
      ...
      parse_identifier(text, &schemaname, &objectname, &schemaquoted, &objectquoted);
    }
    ...
    if (schema_query) {
      ...
      if (schemaname == NULL || schema_query->namespace == NULL) {
        ...
        appendPQExpBuffer(&query_buffer, "%s, NULL::pg_catalog.text FROM %s", schema_query->result,
        ...
        appendPQExpBuffer(&query_buffer, "(%s) LIKE '%s'", schema_query->result, e_object_like);
        if (schema_query->viscondition) appendPQExpBuffer(&query_buffer, " AND %s", schema_query->viscondition);
        if (schema_query->refname) {
          Assert(completion_ref_object);
          appendPQExpBuffer(&query_buffer, " AND %s = '%s'", schema_query->refname, e_ref_object);
          if (schema_query->refnamespace && completion_ref_schema)
            appendPQExpBuffer(&query_buffer, " AND %s = nr.oid AND nr.nspname = '%s'", schema_query->refnamespace,
                              e_ref_schema);
          else if (schema_query->refviscondition)
            appendPQExpBuffer(&query_buffer, " AND %s", schema_query->refviscondition);
        }
         ...
         * the input is just "p".
         */
        if (strcmp(schema_query->catname, "pg_catalog.pg_class c") == 0 && strncmp(objectname, "pg_", 3) != 0) {
        ...
        if (schema_query->namespace) {
          ...
          if (strncmp(objectname, "pg_", 3) != 0)
            appendPQExpBufferStr(&query_buffer, " AND n.nspname NOT LIKE E'pg\\\\_%'");
      ...
      appendPQExpBuffer(&query_buffer, simple_query, e_object_like, e_ref_object, e_ref_schema);
    }
  ...
  if (result && PQresultStatus(result) == PGRES_TUPLES_OK) {
    ...
    while (list_index < PQntuples(result)) {
      ...
      if (!PQgetisnull(result, list_index, 0)) item = PQgetvalue(result, list_index, 0);
      if (PQnfields(result) > 1 && !PQgetisnull(result, list_index, 1)) nsp = PQgetvalue(result, list_index, 1);
      list_index++;
       ...
       */
      if (non_empty_object) {
        if (item && !objectquoted && identifier_needs_quotes(item)) continue;
        if (nsp && !schemaquoted && identifier_needs_quotes(nsp)) continue;
      }
   ...
   */
  if (num_schema_only > 0 && num_query_other == 0 && num_keywords == 0) rl_completion_append_character = '\0';
...
static void set_completion_reference(const char *word) {
  ...
  parse_identifier(word, &completion_ref_schema, &completion_ref_object, &schemaquoted, &objectquoted);
}
...
static char *complete_from_files(const char *text, int state) {
  ...
  if (*text == '\'') completion_force_quote = true;
  ...
  if (unquoted_match) {
    struct stat statbuf;
    bool is_dir = (stat(unquoted_match, &statbuf) == 0 && S_ISDIR(statbuf.st_mode) != 0);

    /* Re-quote the result, if needed. */
    ret = quote_if_needed(unquoted_match, " \t\r\n\"`", '\'', *completion_charp, completion_force_quote, pset.encoding);
    if (ret)
      free(unquoted_match);
    else
      ret = unquoted_match;
...
static char *pg_strdup_keyword_case(const char *s, const char *ref) {
  ...
  if (pset.comp_case == PSQL_COMP_CASE_LOWER ||
      ((pset.comp_case == PSQL_COMP_CASE_PRESERVE_LOWER || pset.comp_case == PSQL_COMP_CASE_PRESERVE_UPPER) &&
       islower(first)) ||
      (pset.comp_case == PSQL_COMP_CASE_PRESERVE_LOWER && !isalpha(first))) {
    for (p = ret; *p; p++) *p = pg_tolower((unsigned char)*p);
  } else {
    for (p = ret; *p; p++) *p = pg_toupper((unsigned char)*p);
  }
...
static char *requote_identifier(const char *schemaname, const char *objectname, bool quote_schema, bool quote_object) {
  ...
  if (schemaname) {
    ...
    if (!quote_schema) quote_schema = identifier_needs_quotes(schemaname);
    if (quote_schema) {
  ...
  if (objectname) {
    ...
    if (!quote_object) quote_object = identifier_needs_quotes(objectname);
    if (quote_object) {
      buflen += 2; /* account for quote marks */
...
static bool identifier_needs_quotes(const char *ident) {
  ...
  /* Check syntax. */
  if (!((ident[0] >= 'a' && ident[0] <= 'z') || ident[0] == '_')) return true;
  if (strspn(ident, "abcdefghijklmnopqrstuvwxyz0123456789_") != strlen(ident)) return true;
  ...
  if (kwnum >= 0 && ScanKeywordCategories[kwnum] != UNRESERVED_KEYWORD) return true;
...
static PGresult *exec_query(const char *query) {
  ...
  if (query == NULL || !pset.db || PQstatus(pset.db) != CONNECTION_OK) return NULL;
...
static char **get_previous_words(int point, char **buffer, int *nwords) {
  ...
  *buffer = outptr = (char *)pg_malloc(point * 2);
   ...
   */
  for (i = point - 1; i >= 0; i--) {
    if (strchr(WORD_BREAKS, buf[i])) break;
  }
  point = i;
  ...
  while (point >= 0) {
    ...
    for (start = end; start > 0; start--) {
      if (buf[start] == '"') inquotes = !inquotes;
      if (!inquotes) {
        if (buf[start] == ')')
          parentheses++;
        else if (buf[start] == '(') {
  ...
  if (buf != rl_line_buffer) free(buf);
...
static char *get_guctype(const char *varname) {
  ...
  if (PQresultStatus(result) == PGRES_TUPLES_OK && PQntuples(result) > 0) guctype = pg_strdup(PQgetvalue(result, 0, 0));
...
static char *quote_file_name(char *fname, int match_type, char *quote_pointer) {
   ...
   * if it's a directory.
   */
  if (*s == '\'' && completion_last_char != '\'' &&
      (match_type != SINGLE_MATCH || (quote_pointer && *quote_pointer == '\'') || stat(fname, &statbuf) != 0 ||
   ...
   * quote.
   */
  if (quote_pointer && *quote_pointer != '\'') *quote_pointer = '\0';
...
static char *dequote_file_name(char *fname, int quote_char) {
  ...
  if (quote_char == '\'') {
    ...
    unquoted_fname = strtokx(workspace, "", NULL, "'", *completion_charp, false, true, pset.encoding);
    free(workspace);
  } else
    unquoted_fname = strtokx(fname, "", NULL, "'", *completion_charp, false, true, pset.encoding);
